...
return static function(PDO $pdo):void{
 ...
 AND NOT EXISTS(SELECT 1 FROM bdc_scoring_publication_points publication_point JOIN bdc_scoring_publications publication ON publication.id=publication_point.publication_id AND publication.status='published' AND publication.approved_by IS NOT NULL AND publication.division=sc.category JOIN bdc_participant_results participant_result ON participant_result.id=publication_point.participant_result_id AND participant_result.competitor_id=sc.competitor_id AND participant_result.dance_style=sc.dance_style WHERE publication_point.competitor_id=sc.competitor_id)");
