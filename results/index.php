...
if($segment==='repository'){
 ...
 if($query!==''&&$rows){
  ...
  $summarySql="SELECT c.id,c.exact_name,c.bdc_id,c.photo_url,c.country,
               ...
               GROUP BY c.id,c.exact_name,c.bdc_id,c.photo_url,c.country
...
<style>body{background:#f5f6f8}.results-card{border:0;border-radius:16px;box-shadow:0 8px 28px rgba(15,23,42,.08)}.hero{background:linear-gradient(135deg,#111827,#27272a);color:#fff}.filter-card{margin-top:0}.document-icon{font-size:1.5rem}.points-summary{border-left:4px solid #111827}.participant-summary-photo{width:58px;height:58px;border-radius:50%;object-fit:cover;background:#eceff3;flex:0 0 auto}.points-total{font-size:1.35rem;font-weight:700}.points-label{font-size:.75rem;letter-spacing:.04em;text-transform:uppercase;color:#6b7280}@media(max-width:575.98px){.participant-summary-photo{width:50px;height:50px}.points-total{font-size:1.15rem}}</style>
</head>
<body>
...
<main class="container pb-5">
 ...
 <?php foreach($participantSummaries as $summary):$intermediatePoints=(float)$summary['intermediate_points'];$novicePoints=(float)$summary['novice_points'];?>
  <div class="col-12 col-lg-6"><div class="border rounded-3 p-3 h-100"><div class="d-flex justify-content-between align-items-start gap-3 mb-3"><div class="d-flex align-items-center gap-3"><div class="text-center"><img class="participant-summary-photo d-block" src="<?=e($summary['photo_url']?:url('/public/assets/img/default-competitor.svg'))?>" alt="<?=e($summary['exact_name'])?>"><?php if($flag=country_flag_url($summary['country']??null)):?><img style="width:30px;height:20px;object-fit:cover;border:1px solid #d1d5db;border-radius:3px;margin-top:4px" src="<?=e($flag)?>" alt="<?=e((string)$summary['country'])?> flag"><?php endif;?></div><div><a class="fw-bold text-dark text-decoration-none" href="<?=e(url('/competitor/?id='.(int)$summary['id']))?>"><?=e($summary['exact_name'])?></a><div class="small text-muted"><?=e($summary['bdc_id'])?></div></div></div><span class="badge text-bg-dark">All-time total</span></div><div class="d-flex flex-wrap gap-4"><?php if($intermediatePoints>0):?><div><div class="points-label">Intermediate</div><div class="points-total"><?=e((string)$intermediatePoints)?> points</div></div><?php endif;?><div><div class="points-label">Novice</div><div class="points-total"><?=e((string)$novicePoints)?> points</div></div></div></div></div>
