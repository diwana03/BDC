    ...
    if ($keepId <= 0 || $mergeId <= 0 || $keepId === $mergeId) {
        ...
        if (!$keep || !$duplicate) {
            ...
            try {
                ...
                // duplicate identity.
                $profileMerge=$pdo->prepare("INSERT INTO bdc_competitor_discipline_profiles(competitor_id,dance_style,dance_role,current_division,special_category) SELECT :keep_id,source.dance_style,source.dance_role,source.current_division,source.special_category FROM bdc_competitor_discipline_profiles source WHERE source.competitor_id=:merge_id ON DUPLICATE KEY UPDATE dance_role=IF(bdc_competitor_discipline_profiles.dance_role='unknown',VALUES(dance_role),bdc_competitor_discipline_profiles.dance_role),current_division=IF(bdc_competitor_discipline_profiles.current_division IN('unknown','novice'),VALUES(current_division),bdc_competitor_discipline_profiles.current_division),special_category=COALESCE(bdc_competitor_discipline_profiles.special_category,VALUES(special_category)),updated_at=NOW()");
