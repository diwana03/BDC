...
try{
 if($_SERVER['REQUEST_METHOD']==='POST'){
  ...
  if($action==='create_scoring_backup'){
   ...
   $roundId=(int)$_POST['round_id'];ScoringRosterCheckpointService::assertEditable($pdo,$roundId,false);$id=(int)$_POST['entry_id'];$pdo->prepare("UPDATE bdc_scoring_entries SET entry_status='withdrawn' WHERE id=:id AND round_id=:r")->execute(['id'=>$id,'r'=>$roundId]);auditScoring($pdo,$roundId,$userId,'entry_removed',['entry_id'=>$id]);$notice='Entry removed.';
