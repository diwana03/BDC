...
try{
 if($_SERVER['REQUEST_METHOD']==='POST'){
  ...
  if($action==='create_scoring_backup'){
   ...
   ScoringRosterCheckpointService::assertEditable($pdo,(int)($_POST['round_id']??0),false);
   ...
   if($entryMode!=='create'){
    ...
    if(!$comp){
     $c=$pdo->prepare("SELECT id,bdc_id,exact_name,dance_role,current_division,status,novice_manual_out,intermediate_manual_out FROM bdc_competitors WHERE LOWER(exact_name) LIKE LOWER(:like) AND dance_role=:role ORDER BY exact_name,id LIMIT 2");
   ...
   $roundId=(int)$_POST['round_id'];ScoringRosterCheckpointService::assertEditable($pdo,$roundId,false);$id=(int)$_POST['entry_id'];$entry=$pdo->prepare("SELECT dance_role FROM bdc_scoring_entries WHERE id=:id AND round_id=:r AND entry_status='active'");$entry->execute(['id'=>$id,'r'=>$roundId]);$role=(string)$entry->fetchColumn();if(!in_array($role,['leader','follower'],true))throw new RuntimeException('Active competitor not found.');$pdo->beginTransaction();try{$pdo->prepare("UPDATE bdc_scoring_entries SET entry_status='withdrawn' WHERE id=:id AND round_id=:r")->execute(['id'=>$id,'r'=>$roundId]);$ids=$pdo->prepare("SELECT id FROM bdc_scoring_entries WHERE round_id=:r AND dance_role=:role AND entry_status='active' ORDER BY bib_number,id");$ids->execute(['r'=>$roundId,'role'=>$role]);$ordered=array_map('intval',$ids->fetchAll(PDO::FETCH_COLUMN));$pdo->prepare("UPDATE bdc_scoring_entries SET bib_number=bib_number+1000000 WHERE round_id=:r AND dance_role=:role AND entry_status='active'")->execute(['r'=>$roundId,'role'=>$role]);$renumber=$pdo->prepare("UPDATE bdc_scoring_entries SET bib_number=:bib WHERE id=:id AND round_id=:r");foreach($ordered as $position=>$entryId)$renumber->execute(['bib'=>$position+1,'id'=>$entryId,'r'=>$roundId]);auditScoring($pdo,$roundId,$userId,'entry_removed',['entry_id'=>$id,'role'=>$role,'roster_renumbered'=>true]);$pdo->commit();}catch(Throwable $e){if($pdo->inTransaction())$pdo->rollBack();throw $e;}$notice='Entry removed and '.ucfirst($role).' bibs reordered.';
  }elseif($action==='save_competitors'){
   $roundId=(int)$_POST['round_id'];ScoringRosterCheckpointService::checkpoint($pdo,$roundId,$userId,'save',false);$notice='Competitor roster checkpoint saved as draft.';
  }elseif($action==='submit_competitors'){
   $roundId=(int)$_POST['round_id'];ScoringRosterCheckpointService::checkpoint($pdo,$roundId,$userId,'submit',false);$notice='Competitors submitted and locked. Manual score entry is now open.';
  }elseif($action==='reopen_competitors'){
   $roundId=(int)$_POST['round_id'];if(!Auth::isSuperAdmin())throw new RuntimeException('Only the Super Admin can reopen submitted competitors.');ScoringRosterCheckpointService::checkpoint($pdo,$roundId,$userId,'reopen',false,(string)($_POST['reopen_reason']??''));$notice='Competitor roster reopened for correction. Scoring is locked until the roster is submitted again.';
   ...
   $notice='Judges saved and linked to the Judge Database. Existing scores for unchanged assignments were preserved.'.($saved['results_invalidated']?' Because the committed panel changed, previous calculated results were cleared; every newly added judge must score before recalculation.':'');
   ...
   if((ScoringRosterCheckpointService::state($pdo,$sourceId,false)['status']??'draft')!=='submitted')throw new RuntimeException('Submit and lock the competitor roster first.');
   $countStmt=$pdo->prepare("SELECT dance_role,COUNT(*) total FROM bdc_scoring_entries WHERE round_id=:round AND entry_status='active' GROUP BY dance_role");$countStmt->execute(['round'=>$sourceId]);$counts=['leader'=>0,'follower'=>0];foreach($countStmt->fetchAll() as $countRow)$counts[(string)$countRow['dance_role']]=(int)$countRow['total'];
   ...
   if(in_array((string)$round['round_type'],['heats','semifinal'],true) && (ScoringRosterCheckpointService::state($pdo,$roundId,false)['status']??'draft')!=='submitted')throw new RuntimeException('Submit and lock the competitor roster before entering or calculating scores.');
   ...
   $pairStmt=$pdo->prepare("SELECT id FROM bdc_scoring_final_pairs WHERE round_id=:r AND (leader_entry_id=:leader_entry OR follower_entry_id=:follower_entry)");
...
$competitorSuggestions=$round?JackJillCompetitorEligibilityService::directory($pdo,(string)$round['dance_style'],null,1500):[];
if($round && in_array((string)$round['round_type'],['heats','semifinal'],true) && !SpecialCategoryService::isSpecial((string)$round['division'])){
 applyAutomaticTier($pdo,$roundId,false);
 $round=loadRound($pdo,$roundId);
}
$rosterState=$round?ScoringRosterCheckpointService::state($pdo,$roundId,false):['status'=>'draft','saved_at'=>null];
$rosterSubmitted=($rosterState['status']??'draft')==='submitted';
...
if($round && ($round['scoring_mode']??'manual')==='automated' && $round['round_type']!=='final'){
 ...
 </main><script src="../../public/js/judge-order-controls.js?v=380"></script></body></html><?php exit;
}
...
<div class="card shadow-sm mb-4 border-2 <?=$registrationDeskInherited?'border-secondary bg-light text-secondary':'border-warning bg-warning-subtle'?>" <?=$registrationDeskInherited?'aria-disabled="true"':''?> id="<?=$registrationDeskInherited?'registration-desk-inherited':'registration-desk-sync'?>">
 <div class="card-header d-flex justify-content-between align-items-center">
  ...
  <span class="badge <?=$registrationDeskInherited?'text-bg-secondary':'text-bg-warning'?>"><?=$registrationDeskInherited?'INHERITED ROUND':'LIVE SYNC'?></span>
 </div>
 <div class="card-body">
...
<?php endif;?>
<?php if($round):?>
<div class="card shadow-sm mb-4 border-primary border-2 bg-primary-subtle"><div class="card-body d-flex justify-content-between align-items-center gap-3 flex-wrap"><div><h2 class="h5 mb-1 text-primary-emphasis">Flights <span class="badge text-bg-primary">Optional</span></h2><div class="text-body-secondary small">Call large rounds in bib-ordered groups. Available for Heats, Semifinal and Final without changing the scoring method.</div></div><a class="btn btn-primary fw-semibold" href="flights.php?round_id=<?=$roundId?>&amp;data_mode=real">Manage Flights</a></div></div>
<?php endif;?>
<?php if($error):?><div class="alert alert-danger"><?=e($error)?></div><?php endif;?><?php if($notice):?><div class="alert alert-success"><?=e($notice)?></div><?php endif;?>
<?php if(!$round):?>
...
<div class="card shadow-sm mb-4 border-dark"><div class="card-body">
 <div class="d-flex justify-content-between align-items-start gap-3 flex-wrap mb-2"><div><h2 class="h5 mb-1">Final Judge Selection</h2><div class="text-muted small">Set the Relative Placement judging panel before matching couples. Search the Judge Database or type a new judge, then select exactly one Chief Judge.</div></div><span class="badge text-bg-dark"><?=($round['scoring_mode']??'manual')==='automated'?'AUTOMATIC FINAL':'MANUAL FINAL'?></span></div>
...
<div class="card shadow-sm mb-4"><div class="card-body">
 ...
 <div class="card border-0 bg-light mb-3"><div class="card-body">
  <h3 class="h6 mb-2">Final Ranking Setup</h3>
 ...
 <form method="post" id="finalScoreForm" <?php if(($round['scoring_mode']??'manual')==='automated'):?>data-final-score-state-url="final-score-state.php?round_id=<?=$roundId?>"<?php endif;?>>
  <input type="hidden" name="_csrf" value="<?=e($csrf)?>">
  <input type="hidden" name="round_id" value="<?=$roundId?>">
...
<?php $rosterCouncil=JackJillCompetitorEligibilityService::council((string)$round['dance_style']);foreach(['leader'=>'Leader','follower'=>'Follower'] as $suggestionRole=>$suggestionLabel):?><datalist id="competitorSuggestions<?=ucfirst($suggestionRole)?>"><?php foreach($competitorSuggestions as $suggestion):if(!in_array((string)$suggestion['dance_role'],[$suggestionRole,'both'],true))continue;?><option value="<?=e($suggestion['identity_code'])?>"><?=e($suggestion['exact_name'].' · '.$suggestionLabel)?></option><?php endforeach;?></datalist><?php endforeach;?>
...
<div class="col-9"><input class="form-control" name="competitor_search" list="competitorSuggestionsLeader" placeholder="Type leader name or <?=$rosterCouncil?> ID" required><div class="form-text">Only active <?=$rosterCouncil?> Leader profiles are shown.</div></div>
<div class="col-12"><button class="btn btn-primary w-100" name="entry_mode" value="existing">Add Existing <?=$rosterCouncil?> Competitor</button></div>
</form>
...
<div class="col-9"><input class="form-control" name="competitor_search" list="competitorSuggestionsFollower" placeholder="Type follower name or <?=$rosterCouncil?> ID" required><div class="form-text">Only active <?=$rosterCouncil?> Follower profiles are shown.</div></div>
<div class="col-12"><button class="btn btn-danger w-100" name="entry_mode" value="existing">Add Existing <?=$rosterCouncil?> Competitor</button></div>
</form>
