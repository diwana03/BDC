...
<details class="card shadow-sm border-primary mt-3 mb-3" id="scoring-backups">
 <summary class="card-header bg-white d-flex justify-content-between align-items-center flex-wrap gap-2" style="cursor:pointer;list-style:none">
  ...
  <span class="d-flex align-items-center gap-2"><a class="btn btn-outline-primary btn-sm" href="<?=e(url('admin/scoring-backups/?data_mode='.($backupTestMode?'test':'live').'&round_id='.$roundId))?>">Open Backups &amp; Recovery</a><span class="badge text-bg-<?=$backupTestMode?'warning':'primary'?>"><?=$backupTestMode?'TEST':'LIVE'?></span></span>
 </summary>
 <div class="card-body pt-3">
  ...
  <div class="table-responsive mb-4"><table class="table table-sm table-bordered align-middle mb-0">
   ...
   <?php foreach($scoringBackups as $backup):$summary=json_decode((string)$backup['summary_json'],true)?:[];?><tr>
    ...
    <td><?php if(!empty($backup['restored_at'])):?><div class="badge text-bg-success mb-1">Last restored <?=e((string)$backup['restored_at'])?></div><?php endif;?><?php if(Auth::canManageScoringBackups()):?><div class="d-flex gap-1 flex-wrap"><details><summary class="btn btn-danger btn-sm">Restore</summary><form method="post"<?=$backupActionAttribute?> class="border rounded bg-light p-2 mt-2" onsubmit="return confirm('Restore checkpoint #<?=(int)$backup['id']?> now? The current scoring state will first be backed up automatically.');"><input type="hidden" name="_csrf" value="<?=e($csrf)?>"><input type="hidden" name="round_id" value="<?=$roundId?>"><input type="hidden" name="backup_id" value="<?=(int)$backup['id']?>"><input type="hidden" name="action" value="restore_scoring_backup"><?php if($backupTestMode):?><input type="hidden" name="test_mode" value="<?=e($testMode??'manual')?>"><?php endif;?><div class="small mb-2"><strong>This restores:</strong> marks, calculated results, Final pairing, Final placements and judge submitted/draft locks shown in this checkpoint.</div><input class="form-control form-control-sm mb-2" name="restore_reason" maxlength="500" required placeholder="Why are you restoring this checkpoint?"><input class="form-control form-control-sm mb-2" name="restore_confirmation" required placeholder="Type RESTORE SCORES"><button class="btn btn-danger btn-sm w-100">Confirm Restore</button></form></details><details><summary class="btn btn-outline-danger btn-sm">Delete</summary><form method="post"<?=$backupActionAttribute?> class="border border-danger rounded bg-light p-2 mt-2" onsubmit="return confirm('Permanently delete checkpoint #<?=(int)$backup['id']?>? This cannot be undone and does not change the current scores.');"><input type="hidden" name="_csrf" value="<?=e($csrf)?>"><input type="hidden" name="round_id" value="<?=$roundId?>"><input type="hidden" name="backup_id" value="<?=(int)$backup['id']?>"><input type="hidden" name="action" value="delete_scoring_backup"><?php if($backupTestMode):?><input type="hidden" name="test_mode" value="<?=e($testMode??'manual')?>"><?php endif;?><div class="small mb-2"><strong>Permanent deletion:</strong> removes only this checkpoint. Current scores stay unchanged.</div><input class="form-control form-control-sm mb-2" name="delete_reason" maxlength="500" required placeholder="Why are you deleting this checkpoint?"><input class="form-control form-control-sm mb-2" name="delete_confirmation" required placeholder="Type DELETE BACKUP"><button class="btn btn-outline-danger btn-sm w-100">Delete Backup</button></form></details></div><?php else:?><span class="text-muted small">View only</span><?php endif;?></td>
