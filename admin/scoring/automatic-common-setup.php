...
function bdcRenderAutomaticCommonSetup(int $roundId):string
{
    ...
    foreach($judgeDirectory as $directoryJudge){$name=(string)($directoryJudge['display_name']?:$directoryJudge['full_name']);$html.='<option value="'.$e($name).'" data-judge-id="'.(int)$directoryJudge['id'].'">'.$e((string)$directoryJudge['judge_code'].(!empty($directoryJudge['country'])?' · '.$directoryJudge['country']:'')).'</option>';}
    $html.='</datalist>';
    $html.='<script>(function(){const form=document.querySelector("#judge-setup form"),wrap=document.getElementById("judgesWrap"),list=document.getElementById("judgeDirectorySuggestions");if(!form||!wrap||!list)return;const master=[...list.options].map(option=>({value:option.value,label:option.label||option.textContent||"",id:option.dataset.judgeId||"0"})),normalise=value=>value.trim().replace(/\\s+/g," ").toLocaleLowerCase(),message=document.createElement("div");message.className="alert alert-danger py-2 mt-2 d-none";message.setAttribute("role","alert");message.textContent="The same judge cannot be selected more than once.";wrap.after(message);function refresh(){const inputs=[...wrap.querySelectorAll("input[name=\\"judge_name[]\\"]")],counts=new Map();inputs.forEach(input=>{const key=normalise(input.value);if(key)counts.set(key,(counts.get(key)||0)+1)});let duplicate=false;inputs.forEach(input=>{const key=normalise(input.value),repeated=key!==""&&(counts.get(key)||0)>1;input.setCustomValidity(repeated?"The same judge cannot be selected more than once.":"");input.classList.toggle("is-invalid",repeated);duplicate=duplicate||repeated;const row=input.closest(".judge-row"),directory=row&&row.querySelector("input[name=\\"judge_directory_id[]\\"]"),match=master.find(item=>normalise(item.value)===key);if(directory)directory.value=match?match.id:"0"});message.classList.toggle("d-none",!duplicate);const selected=new Set(inputs.map(input=>normalise(input.value)).filter(Boolean));list.replaceChildren();master.forEach(item=>{if(selected.has(normalise(item.value)))return;const option=document.createElement("option");option.value=item.value;option.label=item.label;option.dataset.judgeId=item.id;list.appendChild(option)});const submit=form.querySelector("button:not([type=\\"button\\"])");if(submit)submit.disabled=duplicate}wrap.addEventListener("input",refresh);wrap.addEventListener("change",refresh);new MutationObserver(refresh).observe(wrap,{childList:true});form.addEventListener("submit",event=>{refresh();if(!form.checkValidity()){event.preventDefault();form.reportValidity()}});refresh()})();</script>';
