...
function bdcRenderAutomaticCommonSetup(int $roundId):string
{
    ...
    if($special){
        ...
        $html.='<div class="border rounded p-3 bg-light"><div class="fw-semibold mb-2">Heats Judge Choices · Locked</div><div class="row g-2 text-center"><div class="col-3"><small class="text-muted d-block">YES</small><strong>10</strong></div><div class="col-3"><small class="text-muted d-block">ALT 1</small><strong>4.5</strong></div><div class="col-3"><small class="text-muted d-block">ALT 2</small><strong>4.3</strong></div><div class="col-3"><small class="text-muted d-block">ALT 3</small><strong>4.2</strong></div></div></div>';
        $html.='<div class="small text-muted mt-2">Participant-count tiers do not change special-category points. Judge choices and callback workflow remain the same as normal Heats.</div>';
    }else{
        $html.='<form method="post" class="row g-3"><input type="hidden" name="_csrf" value="'.$e($csrf).'"><input type="hidden" name="action" value="settings"><input type="hidden" name="round_id" value="'.$roundId.'">';
