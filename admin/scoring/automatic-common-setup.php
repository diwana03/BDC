...
function bdcRenderAutomaticCommonSetup(int $roundId):string
{
    ...
    $roundDance=JackJillCompetitorEligibilityService::dance((string)$round['dance_style']);
    $council=JackJillCompetitorEligibilityService::council($roundDance);
    $identitySelect=$roundDance==='salsa'
        ? "(SELECT s.sdc_id FROM bdc_sdc_competitors s WHERE s.competitor_id=c.id AND s.status='active' ORDER BY s.id LIMIT 1)"
        : 'c.bdc_id';
    $entryStmt=$pdo->prepare("SELECT se.*,{$identitySelect} identity_code,c.status competitor_status FROM bdc_scoring_entries se JOIN bdc_competitors c ON c.id=se.competitor_id WHERE se.round_id=:round AND se.entry_status='active' ORDER BY se.dance_role,se.bib_number");
    ...
    if($special){
        ...
        $roleConfig=RoleYesConfigurationService::resolve($pdo,$roundId,(int)$round['yes_count']);
