...
function approvalPairs(PDO $pdo,int $roundId,int $tier):array{
 $stmt=$pdo->prepare("
  ...
  JOIN bdc_scoring_final_results fr
...
function approvalAudit(PDO $pdo,int $roundId,int $userId,string $action,array $details=[]):void{
 $stmt=$pdo->prepare("
  INSERT INTO bdc_scoring_audit(round_id,user_id,action,details_json)
...
function refreshPublishedArchivedHtml(PDO $pdo,int $roundId,int $publicationId,int $userId,string $documentTable):array{
 ...
 $documentStmt=$pdo->prepare("SELECT m.document_category,d.id,d.storage_path,d.url
  ...
  JOIN bdc_scoring_publications p ON p.id=m.publication_id AND p.event_id=d.event_id
 ...
 try{
  ...
  $pdo->prepare("UPDATE bdc_scoring_publications SET repository_document_id=:document_id,report_url=:report_url WHERE id=:publication_id AND final_round_id=:round_id AND status='published'")
   ->execute(['document_id'=>$finalDocument['id'],'report_url'=>$finalUrl,'publication_id'=>$publicationId,'round_id'=>$roundId]);
  $pdo->prepare("UPDATE bdc_scoring_rounds SET published_document_id=:document_id WHERE id=:round_id")
 ...
 if($_SERVER['REQUEST_METHOD']==='POST'){
  ...
  if($action==='submit_for_approval'){
   ...
   try{
    if($publication){
     $pdo->prepare("
      UPDATE bdc_scoring_publications
     ...
     $pdo->prepare("
      INSERT INTO bdc_scoring_publications(
  ...
  if($action==='approve_publication'){
   ...
   try{
    ...
    $linkStmt=$pdo->prepare("
     INSERT INTO bdc_scoring_publication_points(
    ...
    $ds=$pdo->prepare("
     INSERT INTO bdc_result_documents(
    ...
    $pdo->prepare("UPDATE bdc_events SET status='completed',updated_at=NOW() WHERE id=:event_id")->execute(['event_id'=>$round['event_id']]);
  ...
  if($action==='refresh_result_archives'){
   ...
   refreshPublishedArchivedHtml($pdo,$roundId,(int)$publication['id'],$userId,'bdc_result_documents');
  ...
  if($action==='rollback'){
   ...
   $linksStmt=$pdo->prepare("
    SELECT *
    FROM bdc_scoring_publication_points
   ...
   try{
    ...
    $pdo->prepare("
     DELETE FROM bdc_scoring_publication_points
     WHERE publication_id=:publication_id
    ...
    foreach($publicationDocumentRows as $d){$pdo->prepare("DELETE FROM bdc_result_documents WHERE id=:id")->execute(['id'=>$d['repository_document_id']]);}
