...
<?php if($deskUrl!==''):?><div class="input"><input id="deskUrl" readonly value="<?=htmlspecialchars($deskUrl)?>"><button type="button" class="btn" onclick="navigator.clipboard.writeText(document.getElementById('deskUrl').value)">Copy Link</button><a class="btn primary" href="<?=htmlspecialchars($deskUrl)?>" target="_blank" rel="noopener">Open Registration Desk</a></div><?php else:?><div class="warn">The secure Registration Desk token is hidden in this admin session. Open the existing desk from the session where its link was generated.</div><?php endif;?>
...
<script>
let deskRefreshBusy=false;async function refreshDesk(){if(document.hidden||deskRefreshBusy)return;deskRefreshBusy=true;try{const r=await fetch('registration-sync.php?round_id=<?=$roundId?>',{cache:'no-store',headers:{'X-Requested-With':'XMLHttpRequest'}});if(r.status===429)return;const d=await r.json();if(!d.ok)return;document.querySelector('[data-stat="leaders"]').textContent=d.leaders_ready+' / '+d.leaders_total;document.querySelector('[data-stat="followers"]').textContent=d.followers_ready+' / '+d.followers_total;document.querySelector('[data-stat="missing"]').textContent=d.missing_bibs;document.querySelector('[data-stat="updated"]').textContent=d.last_update||'No desk changes yet';}catch(e){}finally{deskRefreshBusy=false}}refreshDesk();setInterval(refreshDesk,10000);
</script></body></html>
