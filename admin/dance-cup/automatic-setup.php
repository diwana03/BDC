...
try{
    ...
    if($_SERVER['REQUEST_METHOD']==='POST'){
        ...
        if($action==='add_competitor'){
            ...
            $q=$pdo->prepare("INSERT INTO {$prefix}_entries(competition_id,bib_number,display_name) VALUES(:competition,:number,:name)");
            $q->execute(['competition'=>$id,'number'=>$number,'name'=>$name]);$notice='Contestant added to Automatic Scoring.';
        }elseif($action==='add_judge'){
            $name=trim((string)($_POST['judge_name']??''));if($name==='')throw new RuntimeException('Judge name is required.');
            ...
            $q=$pdo->prepare("INSERT INTO {$prefix}_judges(competition_id,judge_name,judge_order,is_chief) VALUES(:competition,:name,(SELECT COALESCE(MAX(j.judge_order),0)+1 FROM {$prefix}_judges j WHERE j.competition_id=:same),:chief)");
            $q->execute(['competition'=>$id,'name'=>$name,'same'=>$id,'chief'=>!empty($_POST['is_chief'])?1:0]);$notice='Judge added to Automatic Scoring.';
