...
if($_SERVER['REQUEST_METHOD']==='POST'){
    ...
    if($action==='show_entry'){
        ...
        if(!in_array($effect,['none','hearts','balloons','heart_smiles','finger_hearts','gold_rain','fireworks','champion_impact'],true))throw new RuntimeException('Invalid presentation effect.');
...
<div><h3 class="h6">Presentation Effects</h3><div class="effect-grid"><?php foreach(['hearts'=>'💖 Hearts','balloons'=>'🎈 Balloons','heart_smiles'=>'🥰 Smiling Hearts','finger_hearts'=>'🫰 Finger Hearts','gold_rain'=>'✨ Gold Celebration','fireworks'=>'🎆 Fireworks','champion_impact'=>'🏆 Champion Impact','none'=>'Clear Effect'] as $effect=>$label):?><form method="post"><input type="hidden" name="_csrf" value="<?=e($csrf)?>"><input type="hidden" name="id" value="<?=$id?>"><input type="hidden" name="data_mode" value="<?=$test?'test':'real'?>"><input type="hidden" name="effect_type" value="<?=e($effect)?>"><button class="btn <?=$effect==='none'?'btn-outline-secondary':'btn-outline-danger'?>" name="action" value="effect"><?=e($label)?></button></form><?php endforeach;?></div></div>
</div>
</div>
</section>
<div class="row g-4">
