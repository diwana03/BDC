...
$fetch=function(string $sql,string $scope)use($pdo,$competition):array{try{$q=$pdo->prepare($sql);$q->execute(['competition'=>$competition]);return $q->fetchAll();}catch(Throwable $e){error_log('BDC Dance Cup projector '.$scope.' feed fallback: '.$e->getMessage());return [];}};
