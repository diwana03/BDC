...
$wdcCountry="COALESCE(NULLIF(w.country,''),c.country)";
$wdcCountries="CASE WHEN NULLIF(w.country,'') IS NOT NULL THEN NULLIF(w.countries_json,'') ELSE c.countries_json END";
...
$identityRevision=0;
try{
    $identityRevisionQuery=$pdo->prepare("SELECT COALESCE(SUM(CRC32(CONCAT_WS('|',e.id,COALESCE(e.wdc_identity_id,0),COALESCE({$wdcPhoto},''),COALESCE({$wdcCountry},''),COALESCE({$wdcCountries},'')))),0) FROM {$p}_entries e LEFT JOIN bdc_wdc_identities w ON w.id=e.wdc_identity_id LEFT JOIN bdc_competitors c ON c.id=e.competitor_id WHERE e.competition_id=:competition");
    $identityRevisionQuery->execute(['competition'=>$competition]);
    $identityRevision=(int)$identityRevisionQuery->fetchColumn();
}catch(Throwable $e){error_log('BDC Dance Cup projector identity revision fallback: '.$e->getMessage());}
$revision=hash('sha256',json_encode([(int)($state['state_version']??0),$revisionParts,$identityRevision],JSON_UNESCAPED_SLASHES));
...
foreach($results as &$result){
    ...
    if($identity){
        if(!empty($identity['photo_url']))$result['photo_url']=$identity['photo_url'];
