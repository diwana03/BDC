 ...
 if($_SERVER['REQUEST_METHOD']==='POST'){
  ...
  if($action==='reopen_completed_round'){
   ...
   if(($roundForFinal['scoring_mode']??'manual')==='automated'){
    $lockedStmt=$pdo->prepare("SELECT judge_id FROM bdc_test_scoring_judge_sessions WHERE round_id=:r AND status='submitted'");$lockedStmt->execute(['r'=>$roundId]);$lockedJudgeIds=array_map('intval',$lockedStmt->fetchAll(PDO::FETCH_COLUMN));
