...
$showArchived=Auth::isSuperAdmin()&&($_GET['archived']??'0')==='1';
...
try{
 if($_SERVER['REQUEST_METHOD']==='POST'){
  ...
  if($action==='create_scoring_backup'){
   ...
   $roundId=(int)($_POST['round_id']??0);
   $recovery=App\Services\InjuredFinalistRecoveryService::recover($pdo,$roundId,(int)($_POST['entry_id']??0),true,$userId,trim((string)($_POST['recovery_reason']??'')),trim((string)($_POST['recovery_confirmation']??'')),!empty($_POST['promote_next']));
   $notice=$recovery['withdrawn'].' withdrawn from this Test Final. '.($recovery['replacement']?'Next ranked replacement promoted: '.$recovery['replacement'].'. ':'No replacement was promoted; balance the Test Final roster before rematching. ').'Test Final judges are reopened, old pairings are cleared, and the projector is safely on Holding.';
...
$roundsStmt=$pdo->prepare("
 ...
 WHERE r.scoring_mode=:mode AND r.status".($showArchived?"='archived'":"<>'archived'")."
 ORDER BY COALESCE(r.archived_at,r.updated_at) DESC
 LIMIT 30
");
$roundsStmt->execute(['mode'=>$testMode]);
$rounds=$roundsStmt->fetchAll();
$archivedCountStmt=$pdo->prepare("SELECT COUNT(*) FROM bdc_test_scoring_rounds WHERE status='archived' AND scoring_mode=:mode");
$archivedCountStmt->execute(['mode'=>$testMode]);$archivedCount=(int)$archivedCountStmt->fetchColumn();
...
try{
 ...
 error_log('BDC Test scoring judge directory unavailable: '.$judgeDirectoryError->getMessage());
}
$csrf=Csrf::token();
?>
...
<div class="container-fluid py-4" style="max-width:1600px"><div class="d-flex justify-content-between align-items-start mb-3"><div><h1 class="h3 mb-1">Scoring Tests Dashboard</h1><div class="text-muted">Manual Scoring Engine · Event Round Workflow</div><a class="btn btn-sm btn-danger mt-2" href="system-test.php">Run System Test</a></div>
...
<?php if($round):?><span class="badge text-bg-primary status-pill"><?=e(str_replace('_',' ',$round['status']))?></span><?php endif;?></div><?php if($error):?><div class="alert alert-danger"><?=e($error)?></div><?php endif;?><?php if($notice):?><div class="alert alert-success"><?=e($notice)?></div><?php endif;?><?php if(isset($_GET['event_duplicated'])):?><div class="alert alert-success">Test Jack &amp; Jill event duplicated as a fresh Draft. Competitors and judges were copied; scores, results and approvals were reset.</div><?php endif;?><?php if(isset($_GET['duplicate_error'])):?><div class="alert alert-danger"><?=e((string)$_GET['duplicate_error'])?></div><?php endif;?>
...
<?php if(!$round):?>
<div class="card shadow-sm mb-4 border-primary"><div class="card-body"><h2 class="h5 mb-1">Duplicate Test Jack &amp; Jill Event</h2><p class="text-muted">Copies setup, competitors and judges only. Test scores, results, approvals and live state start empty.</p><form method="post" action="duplicate-event.php" class="row g-2" onsubmit="return confirm('Duplicate this Test event with competitors and judges, but without scores or results?');"><input type="hidden" name="_csrf" value="<?=e($csrf)?>"><input type="hidden" name="test_mode" value="<?=e($testMode)?>"><div class="col-md-8"><select class="form-select" name="event_id" required><option value="">Choose Test event to duplicate</option><?php foreach($events as $event):?><option value="<?=(int)$event['id']?>"><?=e(($event['event_date']?:'Date pending').' — '.$event['name'])?></option><?php endforeach;?></select></div><div class="col-md-4"><button class="btn btn-primary w-100">Duplicate Test Event</button></div></form></div></div>
...
<td data-col="actions" class="text-end">
 <div class="d-inline-flex gap-2">
  <?php if(!$showArchived):?><a class="btn btn-sm btn-outline-dark" href="?legacy=1&amp;test_mode=<?=e($testMode)?>&amp;round_id=<?=$r['id']?>">Open</a>
  <?php if(in_array((string)$r['status'],['draft','discarded'],true) && (int)$r['mark_count']===0 && (int)$r['final_mark_count']===0):?><a class="btn btn-sm btn-outline-primary" href="edit-draft.php?mode=<?=e($testMode)?>&amp;round_id=<?=$r['id']?>">Edit Draft Details</a><?php endif;?>
  <?php if(Auth::isSuperAdmin()):?><form method="post" onsubmit="return confirmDeleteWorkflow(this,'<?=e(addslashes($r['event_name']))?>','<?=e(ucfirst($r['division']))?>');"><input type="hidden" name="_csrf" value="<?=e($csrf)?>"><input type="hidden" name="action" value="delete_scoring_workflow"><input type="hidden" name="event_id" value="<?=$r['event_id']?>"><input type="hidden" name="division" value="<?=e($r['division'])?>"><input type="hidden" name="delete_confirmation" value=""><button class="btn btn-sm btn-outline-danger">Delete Complete Test Workflow</button></form><?php endif;?><?php endif;?>
  <?php if(Auth::isSuperAdmin()):?><form method="post" action="duplicate-event.php" data-saved-round-duplicate onsubmit="return confirm('Duplicate this complete Test Jack &amp; Jill event with competitors and judges, but without scores or results?');"><input type="hidden" name="_csrf" value="<?=e($csrf)?>"><input type="hidden" name="test_mode" value="<?=e($testMode)?>"><input type="hidden" name="event_id" value="<?=(int)$r['event_id']?>"><button class="btn btn-sm btn-outline-primary">Duplicate Event</button></form><form method="post" action="../scoring/archive-round.php" onsubmit="return confirm('<?=$showArchived?'Restore this Test scoring round to its previous status?':'Archive this Test scoring round? All setup and scoring data will be preserved.'?>');"><input type="hidden" name="_csrf" value="<?=e($csrf)?>"><input type="hidden" name="round_id" value="<?=(int)$r['id']?>"><input type="hidden" name="data_mode" value="test"><input type="hidden" name="mode" value="<?=e($testMode)?>"><input type="hidden" name="archive_action" value="<?=$showArchived?'restore':'archive'?>"><button class="btn btn-sm <?=$showArchived?'btn-outline-success':'btn-outline-secondary'?>"><?=$showArchived?'Restore':'Archive'?></button></form><?php endif;?>
 </div>
</td>
</tr>
...
<?php endif;?>
<?php if($round['round_type']==='final'):?>
<?php $nextRankedState=NextRankedFinalistService::state($pdo,$roundId,true);$finalRecoveryLocked=App\Services\RandomPairingService::scoringStarted($pdo,$roundId,true);?>
<div class="card shadow-sm mb-4"><div class="card-body">
...
<div class="card shadow-sm mb-4 border-danger"><div class="card-body">
 <h2 class="h5 text-danger">Test Injured Finalist Recovery</h2>
 <p class="text-muted">One protected action creates the normal Test scoring backup, clears only this Test Final's marks, results and pairings, withdraws the selected finalist, reopens every Final judge, revokes the old Emcee link and returns the Test projector to Holding. Previous-round results remain unchanged.</p>
 <?php if(Auth::canOverrideCompletedScores()):?>
 <form method="post" class="row g-2 finalist-recovery-form" onsubmit="return confirm('Withdraw this injured finalist and reset this Test Final for a new match? Previous-round results stay unchanged.');">
  ...
  <div class="col-lg-2 d-flex align-items-end"><button class="btn btn-danger w-100">Recover Test Final</button></div>
...
<div class="card shadow-sm mb-4"><div class="card-body">
 <?php $randomMatchLocked=$finalRecoveryLocked;$emceeLink=App\Services\RandomPairingService::activeLink($pdo,$roundId,true);?>
...
<?php $testCouncil=JackJillCompetitorEligibilityService::council((string)$round['dance_style']);$competitorSuggestions=JackJillCompetitorEligibilityService::directory($pdo,(string)$round['dance_style'],null,1500);?><datalist id="competitorSuggestions"><?php foreach($competitorSuggestions as $suggestion):if(isset($activeCompetitorIds[(int)($suggestion['id']??0)]))continue;?><option value="<?=e($suggestion['identity_code'])?>"><?=e($suggestion['exact_name'].' · '.ucfirst($suggestion['dance_role']))?></option><?php endforeach;?></datalist>
<fieldset <?=$rosterSubmitted?'disabled':''?>><div class="row g-3 mb-4">
<div class="col-lg-6"><div class="card shadow-sm role-card"><div class="card-header bg-primary-subtle fw-semibold d-flex justify-content-between align-items-center"><span>Leaders</span><span class="badge text-bg-primary"><?=count($entries['leader'])?></span></div><div class="card-body">
...
<div class="col-9"><input class="form-control" name="competitor_search" list="competitorSuggestions" placeholder="Type competitor name or <?=$testCouncil?> ID" required><div class="form-text">Only active <?=$testCouncil?> profiles are available in Test.</div></div>
<div class="col-12"><button class="btn btn-danger w-100" name="entry_mode" value="existing">Add Existing <?=$testCouncil?> Competitor</button></div>
</form>
...
function addFinalJudge(){
 ...
 d.innerHTML='<span class="input-group-text final-judge-number"></span>'
  ...
  +'<input class="form-control" name="final_judges['+key+'][name]" placeholder="Search or type a new judge" required>'
