...
use App\Services\SystemTestRunService;
...
if($accessToken!==''&&!$approvedAccess){http_response_code(410);?><!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Test Access Expired</title><link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.3/dist/css/bootstrap.min.css" rel="stylesheet"></head><body class="bg-light"><main class="container py-5" style="max-width:720px"><div class="alert alert-warning"><h1 class="h4">Test access expired</h1><p class="mb-3">No system test was started. Request a fresh dashboard approval before running another controlled test.</p><a class="btn btn-dark" href="<?=e(url('admin/scoring-tests/request-access.php'))?>">Request Fresh Test Access</a></div></main></body></html><?php exit;}
...
$error='';$report=null;$parityReport=null;$eventId=(int)($_GET['event_id']??0);$runId=0;
...
if(($_SERVER['REQUEST_METHOD']??'GET')==='POST'){
    ...
    try{
        ...
        if($action==='run_parity'){
            ...
            if($approvedAccess&&!SystemTestAccessService::canStartRun($approvedAccess))throw new RuntimeException('Test access is too close to expiry. Request fresh approval; no parity data was created.');
            unset($_SESSION['bdc_system_test_run_nonce']);
            $startedRun=SystemTestRunService::begin($pdo,'parity',hash('sha256',$runNonce),$userId,$approvedAccess?(int)$approvedAccess['id']:null);$runId=(int)$startedRun['id'];
            if(!$startedRun['created']){header('Location: '.url('admin/scoring-tests/system-test.php?run_id='.$runId.($accessToken!==''?'&access='.rawurlencode($accessToken):'')),true,303);exit;}
        ...
        if($action==='run'){
        if(!hash_equals($runNonce,(string)($_POST['run_nonce']??'')))throw new RuntimeException('This system test was already submitted. Refresh before running another test.');
        if($approvedAccess&&!SystemTestAccessService::canStartRun($approvedAccess))throw new RuntimeException('Test access is too close to expiry. Request fresh approval; no system-test data was created.');
        unset($_SESSION['bdc_system_test_run_nonce']);
        $startedRun=SystemTestRunService::begin($pdo,'isolated',hash('sha256',$runNonce),$userId,$approvedAccess?(int)$approvedAccess['id']:null);$runId=(int)$startedRun['id'];
        if(!$startedRun['created']){header('Location: '.url('admin/scoring-tests/system-test.php?run_id='.$runId.($accessToken!==''?'&access='.rawurlencode($accessToken):'')),true,303);exit;}
        ...
        if($runId>0){SystemTestRunService::complete($pdo,$runId,$action==='run_parity'?$parityReport:$report);header('Location: '.url('admin/scoring-tests/system-test.php?run_id='.$runId.($accessToken!==''?'&access='.rawurlencode($accessToken):'')),true,303);exit;}
    }catch(Throwable $e){$error=$e->getMessage();if($runId>0){SystemTestRunService::fail($pdo,$runId,$error);header('Location: '.url('admin/scoring-tests/system-test.php?run_id='.$runId.($accessToken!==''?'&access='.rawurlencode($accessToken):'')),true,303);exit;}}
}

$requestedRunId=(int)($_GET['run_id']??0);
$accessRequestId=$approvedAccess?(int)$approvedAccess['id']:null;
if($requestedRunId>0){$storedRun=SystemTestRunService::find($pdo,$requestedRunId,$accessRequestId);if(!$storedRun){$error='System test run not found.';}elseif($storedRun['status']==='error'){$error=(string)$storedRun['error_message'];}elseif(is_array($storedRun['report'])){if($storedRun['run_type']==='parity')$parityReport=$storedRun['report'];else{$report=$storedRun['report'];$eventId=(int)($report['event_id']??0);}}}
$recentRuns=SystemTestRunService::recent($pdo,20,$accessRequestId);
...
<section class="card shadow-sm mb-4"><div class="card-header"><strong>Recent controlled runs</strong></div><div class="table-responsive"><table class="table mb-0"><thead><tr><th>Run</th><th>Type</th><th>Status</th><th>Live</th><th>Test</th><th>Started</th><th>Report</th></tr></thead><tbody><?php foreach($recentRuns as $item):?><tr><td>#<?=(int)$item['id']?></td><td><?=e(ucfirst((string)$item['run_type']))?></td><td><span class="badge <?=($item['status']==='passed'?'text-bg-success':($item['status']==='running'?'text-bg-warning':'text-bg-danger'))?>"><?=e(strtoupper((string)$item['status']))?></span></td><td><?=$item['live_event_id']?'Event #'.(int)$item['live_event_id'].' / Round #'.(int)$item['live_round_id']:'—'?></td><td><?=$item['test_event_id']?'Event #'.(int)$item['test_event_id'].' / Round #'.(int)$item['test_round_id']:'—'?></td><td><?=e($item['started_at'])?></td><td><a class="btn btn-sm btn-outline-primary" href="?run_id=<?=(int)$item['id']?><?=$accessToken!==''?'&amp;access='.rawurlencode($accessToken):''?>">Open</a></td></tr><?php endforeach;?><?php if(!$recentRuns):?><tr><td colspan="7" class="text-muted">No durable test runs have been recorded yet.</td></tr><?php endif;?></tbody></table></div></section>
<section class="card shadow-sm"><div class="card-header"><strong>Recent isolated test events</strong></div><div class="table-responsive"><table class="table mb-0"><thead><tr><th>Event</th><th>Status</th><th>Rounds</th><th>Created</th></tr></thead><tbody><?php foreach($recent as $item):?><tr><td><?=e($item['name'])?></td><td><?=e($item['status'])?></td><td><?=(int)$item['rounds']?></td><td><?=e($item['created_at'])?></td></tr><?php endforeach;?><?php if(!$recent):?><tr><td colspan="4" class="text-muted">No system tests have been run yet.</td></tr><?php endif;?></tbody></table></div></section></main></body></html>
