...
if(round<1)return;const manual=doc.getElementById('heatsScoreForm');if(!manual)return;const host=doc.createElement('div');host.id='bdcAutomaticNativeHost';manual.replaceWith(host);
try{const response=await fetch(panelBase+'?round_id='+round+'&test_mode=automated',{credentials:'same-origin',cache:'no-store',headers:{'X-Requested-With':'XMLHttpRequest'}});if(!response.ok)throw new Error('Judge Live Scoring returned HTTP '+response.status);host.innerHTML=await response.text();executeScripts(doc,host)}catch(error){const message=String(error.message||error);host.innerHTML='<div class="alert alert-danger"><strong>Judge Live Scoring could not load.</strong><br>'+message+'</div>';fail('Automatic Test panel failed: '+message)}
}
