...
try {
 ...
 $entryStmt=$pdo->prepare("SELECT dance_role FROM bdc_test_scoring_entries WHERE id=:entry AND round_id=:round AND entry_status='active'");$entryStmt->execute(['entry'=>$entryId,'round'=>$roundId]);$role=(string)$entryStmt->fetchColumn();
 if(!in_array($role,['leader','follower'],true))throw new RuntimeException('Competitor entry not found.');
 $judgeStmt=$pdo->prepare("SELECT scoring_scope FROM bdc_test_scoring_judges WHERE id=:judge AND round_id=:round");$judgeStmt->execute(['judge'=>$judgeId,'round'=>$roundId]);$scope=(string)$judgeStmt->fetchColumn();
 ...
 $stmt=$pdo->prepare("INSERT INTO bdc_test_scoring_marks(round_id,entry_id,judge_id,mark_type,alt_rank,weighted_score,updated_by) VALUES(:round,:entry,:judge,:type,:alt,:weight,:user) ON DUPLICATE KEY UPDATE mark_type=VALUES(mark_type),alt_rank=VALUES(alt_rank),weighted_score=VALUES(weighted_score),updated_by=VALUES(updated_by),updated_at=NOW()");
