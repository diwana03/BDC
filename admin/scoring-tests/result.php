...
$roundStmt=$pdo->prepare("SELECT r.*,e.name AS event_name,e.event_date,e.venue FROM bdc_test_scoring_rounds r JOIN bdc_test_events e ON e.id=r.event_id WHERE r.id=:id");
$roundStmt->execute(['id'=>$roundId]);
$round=$roundStmt->fetch();
if(!$round){http_response_code(404);exit('Round not found.');}

$judgeStmt=$pdo->prepare("SELECT * FROM bdc_test_scoring_judges WHERE round_id=:r ORDER BY is_chief DESC,judge_order,id");
...
$entryStmt=$pdo->prepare("
 ...
 LEFT JOIN bdc_test_scoring_results sr ON sr.round_id=se.round_id AND sr.entry_id=se.id
...
$paginateReport=!$fitAll&&!$summaryOnly&&($judgeCount>7||$largestRoleCount>24);
$judgeChunks=$paginateReport?array_chunk($judges,14):[$judges];
