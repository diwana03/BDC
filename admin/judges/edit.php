...
$has=static fn(string $field,string $value):bool=>in_array($value,explode(',',(string)($judge[$field]??'')),true);$judgeCountries=CountrySetService::fromRow($judge);if($_SERVER['REQUEST_METHOD']==='POST')$judgeCountries=CountrySetService::fromRequest($_POST);$photoSrc=$judge['photo_url']?:url('public/assets/img/default-competitor.svg');$countries=['Singapore','Thailand','China','Hong Kong','Taiwan','Japan','South Korea','Indonesia','Malaysia','Vietnam','Philippines','India','Australia','New Zealand','United States','United Kingdom','Spain','France','Italy','Germany','Netherlands','Portugal','Morocco','Kazakhstan','Peru','Colombia','Costa Rica','Mexico','Canada','Brazil','Argentina','Cuba'];$judgeFlag=CountryFlagService::emoji($judgeCountries[0]??null);
?>
<!doctype html>
<html>
<head>
