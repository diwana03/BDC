...
<div class="row g-4"><div class="col-lg-5"><div class="card border-0 shadow-sm" id="admin-editor"><div class="card-body"><h2 class="h5 mb-1"><?=$editing?'Edit User & Role':'Add Admin User'?></h2><?php if($editing):?><p class="text-muted mb-3">Editing <?=e($editing['full_name'])?> · <?=e($editing['email'])?></p><?php endif;?>
...
<div class="col-lg-7"><div class="card border-0 shadow-sm"><div class="card-body"><h2 class="h5">Assigned Users</h2><div class="table-responsive"><table class="table align-middle"><thead><tr><th>Name</th><th>Email</th><th>Role</th><th>Status</th><th>Last login</th><th></th></tr></thead><tbody><?php if(!$users):?><tr><td colspan="6" class="text-muted">No assigned users yet.</td></tr><?php endif;?><?php foreach($users as $u):?><tr class="<?=$editing&&(int)$editing['id']===(int)$u['id']?'table-primary':''?>"><td><?=e($u['full_name'])?></td><td><?=e($u['email'])?></td><td><span class="badge text-bg-primary"><?=e($managedRoles[$u['role']]??ucwords(str_replace('_',' ',$u['role'])))?></span></td><td><span class="badge <?=$u['status']==='active'?'text-bg-success':'text-bg-secondary'?>"><?=e(ucfirst($u['status']))?></span></td><td><?=e($u['last_login_at']?:'Never')?></td><td><a class="btn btn-sm btn-outline-dark" href="?edit=<?=(int)$u['id']?>#admin-editor">Edit User &amp; Role</a></td></tr><?php endforeach;?></tbody></table></div></div></div></div></div>
