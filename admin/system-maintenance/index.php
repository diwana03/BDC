...
<div class="col-lg-8">
 ...
 <div class="card settings-card shadow-sm mb-4"><div class="card-body">
  ...
  <?php if($backups):?><form id="bulkPortalBackupDelete" method="post" class="border border-danger-subtle rounded p-2 mb-3 d-flex gap-2 align-items-center flex-wrap" onsubmit="const total=document.querySelectorAll('.portal-backup-select:checked').length;if(!total){alert('Select at least one backup file.');return false;}return confirm('Permanently delete '+total+' selected server backup file'+(total===1?'':'s')+'?');"><input type="hidden" name="_csrf" value="<?=e($csrf)?>"><input type="hidden" name="action" value="delete_selected_backups"><button class="btn btn-outline-danger btn-sm">Delete Selected</button><span class="small text-muted">Select old files below, then delete them together.</span></form><?php endif;?>
