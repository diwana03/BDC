...
<?php if($productionRollbackCandidates):?>
<div class="card shadow-sm mb-4 border border-danger-subtle"><div class="card-body p-4">
<div><h2 class="h5 mb-1">Production Recovery</h2><p class="text-muted">Choose a verified previous Production release. A new full Production backup is created before any rollback starts.</p></div>
<div class="table-responsive"><table class="table align-middle mb-0"><thead><tr><th>Current deployment</th><th>Rollback target</th><th>Deployment date</th><th></th></tr></thead><tbody><?php foreach($productionRollbackCandidates as $candidate):?><tr><td><?=e($candidate['current_version'])?></td><td><strong><?=e($candidate['target_version'])?></strong><div class="small sha"><?=e(substr($candidate['target_sha'],0,12))?></div></td><td><?=e((string)$candidate['completed_at'])?></td><td class="text-end"><form method="post" onsubmit="return confirm('ROLL BACK LIVE PRODUCTION from <?=e($candidate['current_version'])?> to <?=e($candidate['target_version'])?>? A full Production backup will be created first.')"><input type="hidden" name="_csrf" value="<?=e($csrf)?>"><input type="hidden" name="action" value="rollback_production"><input type="hidden" name="deployment_job_id" value="<?=(int)$candidate['id']?>"><button class="btn btn-outline-danger btn-sm" <?=$activeJobs===0?'':'disabled'?>>Rollback to <?=e($candidate['target_version'])?></button></form></td></tr><?php endforeach;?></tbody></table></div>
</div></div>
<?php endif;?>
