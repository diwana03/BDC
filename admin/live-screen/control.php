...
if ($_SERVER["REQUEST_METHOD"] === "POST" && Csrf::verify($_POST["_csrf"] ?? null)) {
  try {
    ...
    } elseif ($action === "generate_mobile_remote") {
        $remoteSession=$sessionId>0?LiveDisplaySessionService::byId($pdo,$sessionId,$test):LiveDisplaySessionService::forEvent($pdo,$roundEventId,$test);
        if(!$remoteSession){LiveDisplaySessionService::generate($pdo,$roundEventId,$test,(int)(Auth::user()["id"]??0));$remoteSession=LiveDisplaySessionService::forEvent($pdo,$roundEventId,$test);}
        if(!$remoteSession)throw new RuntimeException("Generate the Live Display link first.");
        MobileProjectionRemoteService::generate($pdo,(int)$remoteSession['id'],$roundEventId,$test,(int)(Auth::user()["id"]??0));
        $notice = "New 12-hour Mobile Projection Remote generated for this event.";
...
try {
    ...
    foreach (($flightSummary['flights'] ?? []) as $flight) {
        $flightNumber = (int)($flight['number'] ?? 0);
        if ($flightNumber > 0) $flightButtons[$flightNumber] = 'Flight Round ' . $flightNumber;
    }
} catch (Throwable) {
    $flightSummary = ['flight_count' => 0, 'flights' => []];
}
$formats = [
    "16:9" => "16:9 Landscape",
    "9:16" => "9:16 Portrait",
