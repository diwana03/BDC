...
elseif (in_array($type, ["judges", "judge_call"], true)):
    ...
    foreach ($items as $x):
        if (array_key_exists("leader_name", $x)): ?><div class="item"><div class="small" style="font-weight:900;letter-spacing:.08em">COUPLE <?= (int) $x["pair_number"] ?></div><div style="display:flex;align-items:center;justify-content:center;gap:clamp(14px,1.5vw,34px);width:100%;margin:.35em 0"><?php foreach ([["name" => $x["leader_name"], "bib" => $x["leader_bib"], "photo" => $x["leader_photo"], "country" => $x["leader_country"]], ["name" => $x["follower_name"], "bib" => $x["follower_bib"], "photo" => $x["follower_photo"], "country" => $x["follower_country"]]] as $person): $fallbackPhoto = $animalPhoto((string) (($person["bib"] ?? "") . "|" . ($person["name"] ?? ""))); $displayPhoto = !empty($person["photo"]) ? (string) $person["photo"] : $fallbackPhoto; ?><div style="display:flex;flex-direction:column;align-items:center;min-width:0;max-width:44%"><img class="photo" src="<?= e($displayPhoto) ?>" onerror="this.onerror=null;this.src='<?= e($fallbackPhoto) ?>'" style="margin-bottom:.25em"><div class="name" style="font-size:.86em"><?= e($person["name"] ?: "Partner pending") ?></div><div class="small">BIB <?= !empty($person["bib"]) ? (int) $person["bib"] : "—" ?></div><?php if (!empty($person["country"])): ?><div class="small flight-country"><?php if ($flagUrl = country_flag_url($person["country"])): ?><img src="<?= e($flagUrl) ?>" alt="<?= e($person["country"]) ?> flag" style="width:clamp(30px,2.25vw,56px);height:clamp(20px,1.5vw,37px);object-fit:cover;border:1px solid rgba(255,255,255,.7);border-radius:4px"><?php endif; ?><?= e($person["country"]) ?></div><?php endif; ?></div><?php endforeach; ?></div></div><?php else:
