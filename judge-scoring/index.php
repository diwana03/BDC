...
function updateCounter(role){const state=selectionState[role]||{yes:0,A1:0,A2:0,A3:0},el=document.querySelector('[data-counter="'+role+'"]');if(!el)return;el.innerHTML='<span class="counter-item counter-yes">YES '+state.yes+' / '+yesLimit+'</span><span class="counter-item counter-a1">A1 '+state.A1+' / 1</span><span class="counter-item counter-a2">A2 '+state.A2+' / 1</span><span class="counter-item counter-a3">A3 '+state.A3+' / 1</span>';el.classList.toggle('over',state.yes>yesLimit||state.A1>1||state.A2>1||state.A3>1);el.style.borderColor=state.yes===yesLimit&&state.A1===1&&state.A2===1&&state.A3===1?'#198754':'';updateAvailability(role)}
...
async function saveChoice(button){
 ...
 finally{if(card)delete card.dataset.saving}
}
...
updateCounter('leader');updateCounter('follower');renderReviewLater();
