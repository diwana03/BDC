...
function updateCounter(role){const state=selectionState[role]||{yes:0,A1:0,A2:0,A3:0},el=document.querySelector('[data-counter="'+role+'"]');if(!el)return;el.textContent='YES '+state.yes+' / '+yesLimit+'   •   A1 '+state.A1+' / 1   •   A2 '+state.A2+' / 1   •   A3 '+state.A3+' / 1';el.classList.toggle('over',state.yes>yesLimit||state.A1>1||state.A2>1||state.A3>1);el.style.borderColor=state.yes===yesLimit&&state.A1===1&&state.A2===1&&state.A3===1?'#198754':'';updateAvailability(role)}
