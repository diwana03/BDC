...
<button type="button" class="criteria-btn" onclick="document.getElementById('criteriaDialog').showModal()">View Criteria</button><dialog id="criteriaDialog" class="criteria-dialog"><h2>Judging Criteria</h2><ul><?php foreach(JudgingCriteriaService::weights() as [$name,$weight]):?><li><strong><?=e($name)?></strong>, <?=e($weight)?></li><?php endforeach;?></ul><p><?php if($isFinal):?>Use Relative Placement. Rank every couple once without duplicate placements.<?php else:?>Select exactly <?=$yesLimit?> YES per assigned role. A1, A2 and A3 may each be used once per role.<?php endif;?></p><p>Judge independently and keep all scoring confidential.</p><button type="button" class="criteria-btn" onclick="document.getElementById('criteriaDialog').close()">Close</button></dialog>
...
function updateSubmitReadiness(){const button=document.getElementById('submitScores');if(!button)return;if(<?=json_encode($isFinal)?>){const ranks=[...document.querySelectorAll('.judge-input')].map(input=>parseInt(input.value||'0',10)),ready=ranks.length>0&&ranks.every(rank=>rank>=1&&rank<=ranks.length)&&new Set(ranks).size===ranks.length;button.disabled=!ready;button.textContent=ready?'READY TO SUBMIT FINAL':'COMPLETE ALL RANKS';return}const ready=['leader','follower'].every(role=>roleReady(role));button.disabled=!ready;button.textContent=ready?'READY TO SUBMIT':'COMPLETE REQUIRED SELECTIONS'}
