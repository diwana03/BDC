...
<style>.submit:disabled{background:#8A94A3;color:#F8F9FA;cursor:not-allowed;opacity:1}.submit:not(:disabled){background:#146C43;color:#fff;box-shadow:0 0 0 3px rgba(20,108,67,.16)}</style>
...
<div class="card"><strong>Private draft ranking</strong><div class="small">Use this scratchpad before entering official ranks. It stays only on this device.</div><textarea id="rankScratchpad" rows="5" style="width:100%;margin-top:8px;padding:10px;border:1px solid #ccd2da;border-radius:9px" placeholder="Example: 1 = Pair 7, 2 = Pair 3…"></textarea></div>
<?php foreach($pairs as $pair):?><div class="card"><div class="small" style="font-weight:900">PAIR #<?=(int)$pair['pair_number']?></div><div class="bib">BIB <?=(int)$pair['leader_bib']?> + BIB <?=(int)$pair['follower_bib']?></div><div class="name"><?=e($pair['leader_name'])?> &amp; <?=e($pair['follower_name'])?></div><div><label>Rank</label><select class="score judge-input" data-pair="<?=(int)$pair['pair_id']?>" data-label="Pair <?=(int)$pair['pair_number']?>"><option value="">Unranked</option><?php for($rankOption=1;$rankOption<=$finalRankLimit;$rankOption++):?><option value="<?=$rankOption?>" <?=$rankOption===(int)($pair['rank_value']??0)?'selected':''?>><?=$rankOption?></option><?php endfor;?></select><span class="saved"></span></div></div><?php endforeach;?>
...
document.querySelectorAll('.judge-input').forEach(input=>{input.dataset.previous=input.value;input.addEventListener('change',async()=>{const duplicate=[...document.querySelectorAll('.judge-input')].find(other=>other!==input&&input.value!==''&&other.value===input.value);if(duplicate){showRuleMessage('Rank '+input.value+' is already assigned to '+duplicate.dataset.label+'. Choose another rank or clear it first.');input.value=input.dataset.previous;return;}const fake={dataset:{value:input.value,pair:input.dataset.pair},parentElement:input.parentElement,closest:()=>null};await saveChoice(fake);input.dataset.previous=input.value;updateSubmitReadiness()})});
...
updateCounter('leader');updateCounter('follower');renderReviewLater();updateSubmitReadiness();
