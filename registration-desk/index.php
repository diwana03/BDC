...
document.querySelector('#toggleProvisional').addEventListener('click',event=>{const input=document.querySelector('#entryMode'),provisional=input.value!=='create';input.value=provisional?'create':'existing';document.querySelector('#provisionalFields').classList.toggle('d-none',!provisional);document.querySelector('#competitorLabel').textContent=provisional?'Competitor full name':'BDC competitor';event.currentTarget.textContent=provisional?'Return to BDC search':'Competitor missing from BDC';document.querySelector('#competitorMatches').innerHTML='';document.querySelector('#competitorSearch').value=''});
