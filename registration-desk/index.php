...
$roundStmt=$pdo->prepare("SELECT * FROM bdc_scoring_rounds WHERE event_id=:event AND division=:division AND round_type='heats' AND status NOT IN ('archived','completed') ORDER BY id DESC LIMIT 1");
$roundStmt->execute(['event'=>$desk['event_id'],'division'=>$desk['division']]);$round=$roundStmt->fetch();
if(!$round){http_response_code(404);exit('No active Heats round exists for this desk link.');}
...
<header class="desk-head py-3"><div class="container"><h1 class="h4 mb-1">BDC Registration Desk</h1><div><?=e($desk['event_name'])?> · <?=e(ucfirst($desk['division']))?></div></div></header>
