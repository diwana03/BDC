...
) ?>"><link rel="stylesheet" data-bdc-theme-styles href="<?= e(url("public/assets/css/bdc-theme.css?v=420")) ?>"><link rel="stylesheet" href="<?= e(url("public/assets/css/admin-mobile-v428.css?v=428")) ?>"><script defer src="<?= e(url("public/assets/js/bdc-theme.js?v=420")) ?>"></script><script defer src="<?= e(url("public/assets/js/admin-mobile-v428.js?v=428")) ?>"></script></head><body class="admin-v203 <?= e(
    $bdcEnvironmentClass,
...
(function(){
 ...
 if(App\Core\Auth::isSuperAdmin()&&!empty($pendingSystemTestAccess)):?><div class="card border-danger shadow-sm mb-4"><div class="card-header bg-danger text-white"><strong>System Test Access Requests · <?=count($pendingSystemTestAccess)?></strong></div><div class="card-body"><p class="mb-3">Approving grants one browser a 20-minute link to the isolated <code>bdc_test_*</code> runner only. It does not create an Admin login.</p><?php foreach($pendingSystemTestAccess as $request):?><div class="d-flex justify-content-between align-items-center gap-3 border-top py-3"><span><strong>Request #<?=(int)$request['id']?></strong> · token <?=e($request['token_hint'])?>…<small class="d-block text-muted">Requested <?=e($request['requested_at'])?> · expires <?=e($request['expires_at'])?></small></span><form method="post" class="d-flex gap-2"><input type="hidden" name="_csrf" value="<?=e(App\Core\Csrf::token())?>"><input type="hidden" name="action" value="decide_system_test_access"><input type="hidden" name="request_id" value="<?=(int)$request['id']?>"><button class="btn btn-success btn-sm" name="decision" value="approve">Approve Test Only</button><button class="btn btn-outline-danger btn-sm" name="decision" value="deny">Deny</button></form></div><?php endforeach;?></div></div><?php endif;
 ?><section class="admin-page-heading-v203"><div><h1>Dashboard</h1><p>Welcome back. Here is what is happening with your competition portal.</p></div><span class="admin-date-chip-v203"><?= date(
    "j M Y",
) ?></span></section><?php if(!empty($activeBackupAlerts)):?><section class="card border-danger shadow-sm mb-4"><div class="card-header bg-danger text-white d-flex justify-content-between"><strong>Backup System Alert</strong><span><?=count($activeBackupAlerts)?> active</span></div><div class="card-body"><?php foreach($activeBackupAlerts as $alert):?><div class="d-flex justify-content-between align-items-start gap-3 py-2 border-bottom"><div><strong><?=e((string)$alert['title'])?></strong><div class="text-danger"><?=e((string)$alert['message'])?></div><small class="text-muted">Last failure <?=e((string)$alert['last_occurred_at'])?> · <?=(int)$alert['occurrence_count']?> occurrence<?=((int)$alert['occurrence_count']===1?'':'s')?> · <?=e(ucfirst((string)$alert['status']))?></small></div><div class="d-flex gap-2"><?php if($alert['status']==='open'):?><form method="post"><input type="hidden" name="_csrf" value="<?=e(App\Core\Csrf::token())?>"><input type="hidden" name="action" value="acknowledge_backup_alert"><input type="hidden" name="alert_id" value="<?=(int)$alert['id']?>"><button class="btn btn-outline-danger btn-sm">Acknowledge</button></form><?php endif;?><a class="btn btn-danger btn-sm" href="<?=e(url('admin/system-maintenance/'))?>">Open Backups</a></div></div><?php endforeach;?></div></section><?php endif;?><section class="admin-metric-grid-v203"><a class="admin-metric-card-v203 metric-orange" href="<?= e(
    url("admin/competitors/"),
