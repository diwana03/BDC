...
final class ProfileDiagnosticQueryService
{
    public static function run(PDO $pdo,string $query,array $params):array
    {
        ...
        return match($query){
            'bdc_members'=>self::rows($pdo,"SELECT c.id competitor_id,c.bdc_id,c.exact_name,c.photo_url IS NOT NULL AND TRIM(c.photo_url)<>'' photo_present,COALESCE(p.dance_role,c.dance_role,'unknown') bachata_role,COALESCE(p.current_division,c.current_division,'unknown') bachata_division,(SELECT GROUP_CONCAT(sc.category ORDER BY sc.category SEPARATOR ',') FROM bdc_competitor_special_categories sc WHERE sc.competitor_id=c.id AND sc.dance_style='bachata') bachata_categories,(SELECT COUNT(*) FROM bdc_participant_results pr WHERE pr.competitor_id=c.id AND pr.dance_style='bachata') bachata_result_count,(SELECT COUNT(*) FROM bdc_point_transactions pt WHERE pt.competitor_id=c.id AND pt.dance_style='bachata') bachata_point_transaction_count,(SELECT COALESCE(SUM(pt.points),0) FROM bdc_point_transactions pt WHERE pt.competitor_id=c.id AND pt.dance_style='bachata') bachata_points FROM bdc_competitors c LEFT JOIN bdc_competitor_discipline_profiles p ON p.competitor_id=c.id AND p.dance_style='bachata' WHERE c.status<>'archived' AND c.bdc_id IS NOT NULL AND TRIM(c.bdc_id)<>'' ORDER BY LOWER(c.exact_name),c.id LIMIT $limit",[]),
            ...
            default=>throw new RuntimeException('Unknown diagnostic query. Allowed: bdc_members, sdc_members, wdc_members, person_match, missing_photos, competitor_history, deletion_impact.'),
        };
    }
