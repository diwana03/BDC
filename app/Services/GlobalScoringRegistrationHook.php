 ...
 public static function handle(string $method,string $path,string $testMode=''):void
 {
  ...
  try{
   if(!$create){
    ...
    $competitor=CompetitorIdentityService::findOrCreateOfficial($pdo,$name,$role,$initial);
    $danceStyle=in_array((string)($round['dance_style']??'bachata'),['bachata','salsa'],true)?(string)$round['dance_style']:'bachata';
    if(!empty($competitor['created'])){$pdo->prepare("INSERT INTO bdc_competitor_discipline_profiles(competitor_id,dance_style,dance_role,current_division) VALUES(:competitor,:dance,:role,:division) ON DUPLICATE KEY UPDATE dance_role=VALUES(dance_role)")->execute(['competitor'=>$competitor['id'],'dance'=>$danceStyle,'role'=>$role,'division'=>$initial]);}
   ...
   $auditTable=$isTest?'bdc_test_scoring_audit':'bdc_scoring_audit';$audit=$pdo->prepare("INSERT INTO {$auditTable}(round_id,user_id,action,details_json) VALUES(:round,:user,'new_bdc_competitor_registered',:details)");$audit->execute(['round'=>$roundId,'user'=>(int)(Auth::user()['id']??0)?:null,'details'=>json_encode(['competitor_id'=>(int)$competitor['id'],'bdc_id'=>(string)$competitor['bdc_id'],'role'=>$role,'bib'=>$bib,'bib_status'=>$bib===null?'unassigned':'assigned','entered_division'=>(string)$round['division'],'permanent_division_changed'=>false],JSON_UNESCAPED_SLASHES)]);
