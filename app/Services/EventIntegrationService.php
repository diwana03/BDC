...
final class EventIntegrationService
{
    ...
    private static function jackJillPayload(PDO $pdo,array $payload,string $mode):array
    {
        ...
        foreach($rounds as $round){
            ...
            $competitors=self::competitors($pdo,(array)($round['competitors']??[]),$mode,true);
    ...
    private static function danceCupPayload(PDO $pdo,array $payload,string $mode):array
    {
        ...
        foreach($categories as $category){
            ...
            $competitors=self::competitors($pdo,(array)($category['competitors']??[]),$mode,false);$judges=self::judges($pdo,(array)($category['judges']??[]),false);
    ...
    private static function competitors(PDO $pdo,array $rows,string $mode,bool $roles):array
    {
        if(!$rows)throw new RuntimeException('Select at least one competitor.');$table=$mode==='test'?'bdc_test_competitors':'bdc_competitors';$clean=[];$seen=[];$bibs=[];
        foreach($rows as $row){if(!is_array($row))throw new RuntimeException('Every competitor selection must be an object.');$bdc=strtoupper(trim((string)($row['bdc_id']??'')));if(!preg_match('/^BDC-\d+$/',$bdc))throw new RuntimeException('Every competitor requires a BDC ID.');$role=$roles?strtolower(trim((string)($row['role']??''))):'';if($roles&&!in_array($role,['leader','follower'],true))throw new RuntimeException('Jack & Jill competitor role must be leader or follower.');$identity=$bdc."\0".$role;if(isset($seen[$identity]))throw new RuntimeException('The same competitor cannot be selected twice in one category.');$seen[$identity]=true;$bib=self::boundedInt($row['bib']??0,1,999999,'bib');$bibKey=($roles?$role:'all')."\0".$bib;if(isset($bibs[$bibKey]))throw new RuntimeException('Competitor bibs must be unique within each role/category.');$bibs[$bibKey]=true;$q=$pdo->prepare("SELECT id,exact_name FROM {$table} WHERE bdc_id=:bdc AND status<>'archived' LIMIT 1");$q->execute(['bdc'=>$bdc]);$profile=$q->fetch();if(!$profile)throw new RuntimeException($bdc.' was not found in the '.$mode.' competitor directory.');$clean[]=['competitor_id'=>(int)$profile['id'],'bdc_id'=>$bdc,'name'=>(string)$profile['exact_name'],'role'=>$role,'bib'=>$bib];}
        return $clean;
    }
