...
final class DanceCupScoringService
{
    ...
    public static function ensureWorkspaceTables(PDO $pdo, bool $test = false): void
    {
        ...
        $pdo->exec("CREATE TABLE IF NOT EXISTS {$prefix}_judge_comments(competition_id BIGINT UNSIGNED NOT NULL,entry_id BIGINT UNSIGNED NOT NULL,judge_id BIGINT UNSIGNED NOT NULL,private_comment TEXT NOT NULL,updated_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,PRIMARY KEY(competition_id,entry_id,judge_id),INDEX idx_dc_comments_judge(competition_id,judge_id)) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci");
        ...
        $pdo->exec("CREATE TABLE IF NOT EXISTS {$prefix}_tie_tasks(id BIGINT UNSIGNED AUTO_INCREMENT PRIMARY KEY,competition_id BIGINT UNSIGNED NOT NULL,tie_key CHAR(64) NOT NULL,tied_score DECIMAL(12,2) NOT NULL,entry_ids_json TEXT NOT NULL,token_hash CHAR(64) NULL,status VARCHAR(20) NOT NULL DEFAULT 'pending',resolved_order_json TEXT NULL,created_by BIGINT UNSIGNED NULL,chief_judge_assignment_id BIGINT UNSIGNED NULL,created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,expires_at DATETIME NULL,resolved_at DATETIME NULL,cancelled_at DATETIME NULL,UNIQUE KEY uq_dc_tie_key(competition_id,tie_key),INDEX idx_dc_tie_status(competition_id,status)) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci");
        ...
        foreach ([
            "criteria_version CHAR(64) NULL AFTER access_token",
            "criteria_accepted_at DATETIME NULL AFTER criteria_version",
        ] as $definition) {
            try { $pdo->exec("ALTER TABLE {$prefix}_judge_sessions ADD COLUMN ".$definition); } catch (\Throwable) {}
        }
    ...
    public static function approveResults(PDO $pdo,int $competitionId,int $userId,bool $test=false,?string $notes=null):void
    {
        ...
        if(!$test){
            $save=$pdo->prepare("INSERT INTO bdc_dance_cup_result_history(competition_id,event_id,entry_id,competitor_id,wdc_identity_id,display_name,dance_style,entry_type,competition_level,gender_eligibility,placement,total_score,approved_by,approved_at) VALUES(:competition,:event,:entry,:competitor,:wdc,:name,:style,:entry_type,:level,:gender,:placement,:total,:user,NOW()) ON DUPLICATE KEY UPDATE competitor_id=VALUES(competitor_id),wdc_identity_id=VALUES(wdc_identity_id),display_name=VALUES(display_name),placement=VALUES(placement),total_score=VALUES(total_score),approved_by=VALUES(approved_by),approved_at=NOW()");
            $linkEntry=$pdo->prepare("UPDATE {$prefix}_entries SET wdc_identity_id=:wdc WHERE id=:entry AND competition_id=:competition");
            $award=$pdo->prepare("INSERT INTO bdc_wdc_championship_points(wdc_identity_id,competition_id,entry_id,event_id,division,placement,points,awarded_by) VALUES(:wdc,:competition,:entry,:event,:division,:placement,:points,:user) ON DUPLICATE KEY UPDATE wdc_identity_id=VALUES(wdc_identity_id),division=VALUES(division),placement=VALUES(placement),points=VALUES(points),awarded_by=VALUES(awarded_by),awarded_at=NOW()");
            foreach($rows as $result){
                $identity=CouncilResultIdentityService::wdcIdentityForEntry($pdo,(string)$row['entry_type'],(string)$result['display_name'],!empty($result['competitor_id'])?(int)$result['competitor_id']:null);
                $wdcId=(int)$identity['id'];
                $linkEntry->execute(['wdc'=>$wdcId,'entry'=>$result['entry_id'],'competition'=>$competitionId]);
                $save->execute(['competition'=>$competitionId,'event'=>$row['event_id'],'entry'=>$result['entry_id'],'competitor'=>$result['competitor_id']?:null,'wdc'=>$wdcId,'name'=>$result['display_name'],'style'=>$row['dance_style'],'entry_type'=>$row['entry_type'],'level'=>$row['competition_level'],'gender'=>$row['gender_eligibility']??'mixed','placement'=>$result['placement'],'total'=>$result['total_score'],'user'=>$userId]);
                if((string)$row['competition_level']==='open')$award->execute(['wdc'=>$wdcId,'competition'=>$competitionId,'entry'=>$result['entry_id'],'event'=>$row['event_id'],'division'=>'open','placement'=>$result['placement'],'points'=>CouncilResultIdentityService::wdcOpenPoints((int)$result['placement']),'user'=>$userId?:null]);
            }
        }
