...
final class SdcReconciliationService
{
    ...
    public static function directory(PDO $pdo,string $search,int $page,int $perPage):array
    {
        ...
        if($search!==''){$where.=" AND (LOWER(c.exact_name) LIKE LOWER(:search) OR LOWER(c.normalised_name) LIKE LOWER(:search) OR LOWER(COALESCE(c.bdc_id,'')) LIKE LOWER(:search) OR LOWER(COALESCE(sdc.identity_code,'')) LIKE LOWER(:search))";$params['search']='%'.$search.'%';}
        $count=$pdo->prepare("SELECT COUNT(*) FROM bdc_competitors c LEFT JOIN bdc_result_identities sdc ON sdc.competitor_id=c.id AND sdc.council='sdc' WHERE $where");$count->execute($params);
        $sql="SELECT c.id,c.bdc_id,sdc.identity_code sdc_id,c.exact_name,c.normalised_name,c.status,COALESCE(p.dance_role,'unknown') salsa_role,COALESCE(p.current_division,'unknown') salsa_division,c.photo_url,GROUP_CONCAT(DISTINCT sc.category ORDER BY sc.category SEPARATOR ',') salsa_categories FROM bdc_competitors c LEFT JOIN bdc_result_identities sdc ON sdc.competitor_id=c.id AND sdc.council='sdc' LEFT JOIN bdc_competitor_discipline_profiles p ON p.competitor_id=c.id AND p.dance_style='salsa' LEFT JOIN bdc_competitor_special_categories sc ON sc.competitor_id=c.id AND sc.dance_style='salsa' WHERE $where GROUP BY c.id,c.bdc_id,sdc.identity_code,c.exact_name,c.normalised_name,c.status,p.dance_role,p.current_division,c.photo_url ORDER BY LOWER(c.exact_name),c.id LIMIT $perPage OFFSET $offset";
    ...
    private static function plan(PDO $pdo,array $members):array
    {
        ...
        if($ids){$marks=implode(',',array_fill(0,count($ids),'?'));$q=$pdo->prepare("SELECT c.id,c.bdc_id,c.exact_name,c.photo_url,p.current_division FROM bdc_competitors c LEFT JOIN bdc_competitor_discipline_profiles p ON p.competitor_id=c.id AND p.dance_style='salsa' WHERE c.status<>'archived' AND c.id IN ($marks) ORDER BY c.id");$q->execute($ids);foreach($q->fetchAll() as $row){$id=(int)$row['id'];$found[$id]=true;if(trim((string)($row['photo_url']??''))==='')$missingPhotos[]=['competitor_id'=>$id,'bdc_id'=>$row['bdc_id'],'exact_name'=>$row['exact_name']];if((string)$row['current_division']==='novice')$novice++;}}
        $unmatched=array_values(array_filter($ids,static fn(int $id):bool=>!isset($found[$id])));
        $existing=$pdo->query("SELECT ri.competitor_id,c.bdc_id,c.exact_name FROM bdc_result_identities ri JOIN bdc_competitors c ON c.id=ri.competitor_id WHERE ri.council='sdc' ORDER BY ri.competitor_id")->fetchAll();$wanted=array_fill_keys($ids,true);$remove=[];$removeRows=[];foreach($existing as $row)if(!isset($wanted[(int)$row['competitor_id']])){$remove[]=(int)$row['competitor_id'];$removeRows[]=['competitor_id'=>(int)$row['competitor_id'],'bdc_id'=>$row['bdc_id'],'exact_name'=>$row['exact_name']];}
