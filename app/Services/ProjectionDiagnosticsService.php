...
final class ProjectionDiagnosticsService
{
    ...
    private static function danceCup(PDO $pdo,string $mode,int $eventId,int $requestedCompetition,array &$checks):array
    {
        ...
        [$entryRows,$entryError]=self::all($pdo,"SELECT e.id,e.display_name,e.bib_number,e.competitor_id,e.wdc_identity_id,COALESCE(NULLIF(w.country,''),c.country) country,COALESCE(NULLIF(w.photo_url,''),c.photo_url) photo_url FROM {$entries} e LEFT JOIN bdc_wdc_identities w ON w.id=e.wdc_identity_id LEFT JOIN bdc_competitors c ON c.id=e.competitor_id WHERE e.competition_id=:competition AND e.status='active' ORDER BY e.bib_number,e.id",['competition'=>$competitionId]);
        if($entryError!==null){self::check($checks,'roster_query','fail',$entryError);$entryRows=[];}
        $missingBib=[];$missingCountry=[];$invalidCountry=[];$missingPhoto=[];$missingProfile=[];
        foreach($entryRows as $row){$name=(string)($row['display_name']??('entry #'.$row['id']));if((int)($row['bib_number']??0)<1)$missingBib[]=$name;if(empty($row['competitor_id'])&&empty($row['wdc_identity_id']))$missingProfile[]=$name;$country=trim((string)($row['country']??''));if($country==='')$missingCountry[]=$name;elseif(CountryFlagService::code($country)===null)$invalidCountry[]=$name.' ('.$country.')';if(trim((string)($row['photo_url']??''))==='')$missingPhoto[]=$name;}
