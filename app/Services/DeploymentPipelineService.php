...
final class DeploymentPipelineService
{
    ...
    public static function rollbackProduction(PDO $pdo,int $deploymentJobId,int $userId):array
    {
        ...
        try{
            $safetyBackup=self::backupProduction($config,(string)$deployment['commit_sha'],$rollbackOutput);
            $rollbackOutput[]='Current Production was fully backed up before rollback: '.$safetyBackup;
    ...
    public static function productionRollbackCandidates(PDO $pdo):array
    {
        $config=self::settings();
        $stmt=$pdo->query("SELECT j.id,j.completed_at,j.output,r.version current_version
            FROM bdc_deployment_jobs j JOIN bdc_release_candidates r ON r.id=j.release_id
            WHERE j.action='deploy_production' AND j.target_environment='production' AND j.status='success'
            ORDER BY j.completed_at DESC,j.id DESC LIMIT 20");
        $items=[];
        foreach($stmt->fetchAll() as $row){
            if(!preg_match('/(?:\[PRODUCTION_BACKUP\]\s*|Production files backed up to\s+)([^\r\n]+)/',(string)$row['output'],$match))continue;
            $backup=realpath(trim($match[1]));
            $root=realpath($config['backup_path']);
            if($backup===false||$root===false||!str_starts_with($backup.DIRECTORY_SEPARATOR,$root.DIRECTORY_SEPARATOR))continue;
            $manifest=$backup.'/files/storage/release.json';
            $target=is_file($manifest)?json_decode((string)file_get_contents($manifest),true):null;
            if(!is_array($target)||empty($target['version'])||empty($target['commit_sha']))continue;
            $items[]=$row+['target_version'=>(string)$target['version'],'target_sha'=>(string)$target['commit_sha']];
        }
        return $items;
    }
