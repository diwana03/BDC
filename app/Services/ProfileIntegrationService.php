...
final class ProfileIntegrationService
{
    ...
    private static function competitorPayload(array $p):array
    {
        ...
        $primaryCountry=substr(CountryFlagService::canonicalName((string)($p['country']??'')),0,100);$countries=CountrySetService::normalize((array)($p['countries']??[$primaryCountry]));return ['form_kind'=>$kind,'full_name'=>$name,'bdc_id'=>strtoupper(trim((string)($p['bdc_id']??''))),'sdc_id'=>strtoupper(trim((string)($p['sdc_id']??''))),'role'=>$role,'styles'=>$styles,'email'=>$email,'phone'=>trim((string)($p['phone']??'')),'instagram'=>self::instagram((string)($p['instagram']??'')),'country'=>$countries[0]??'','countries'=>$countries,'replace_fields'=>self::allowedList($p['replace_fields']??[],['full_name','email','phone','instagram','country','photo']),'photo_base64'=>(string)($p['photo_base64']??''),'photo_mime'=>(string)($p['photo_mime']??''),'photo_name'=>(string)($p['photo_name']??'')];
    }
    ...
    private static function wdcPayload(array $p):array
    {
        ...
        $countries=CountrySetService::normalize((array)($p['countries']??[$p['country']??'']));return ['wdc_id'=>$wdcId,'entry_type'=>$type,'display_name'=>$name,'person_id'=>$personId,'country'=>$countries[0]??'','countries'=>$countries,'photo_url'=>substr(trim((string)($p['photo_url']??'')),0,1000),'registrations'=>$registrations,'photo_base64'=>(string)($p['photo_base64']??''),'photo_mime'=>(string)($p['photo_mime']??''),'photo_name'=>(string)($p['photo_name']??'')];
    }
    ...
    private static function applyCompetitor(PDO $pdo,array $u,array $p):array
    {
        ...
        if(!empty($p['countries']))$pdo->prepare('UPDATE bdc_competitors SET country=:country,countries_json=:countries WHERE id=:id')->execute(['country'=>$p['countries'][0],'countries'=>CountrySetService::json($p['countries']),'id'=>$id]);
    ...
    private static function applyJudge(PDO $pdo,array $u,array $p):array
    {
        ...
        if(!empty($p['countries']))$pdo->prepare('UPDATE bdc_judges SET country=:country,countries_json=:countries,country_code=:code WHERE id=:id')->execute(['country'=>$p['countries'][0],'countries'=>CountrySetService::json($p['countries']),'code'=>CountryFlagService::code($p['countries'][0]),'id'=>$id]);
    ...
    private static function applyWdc(PDO $pdo,array $u,array $p,int $userId):array
    {
        ...
        if(($p['operation']??'upsert')==='photo_replace'){
            ...
            $pdo->prepare('UPDATE bdc_wdc_identities SET photo_url=:photo,original_photo_url=:original WHERE id=:id')->execute(['photo'=>$photo,'original'=>$photo,'id'=>$id]);
            return ['id'=>$id,'new_photo_path'=>$path];
        }
        ...
        $pdo->prepare("UPDATE bdc_wdc_identities SET display_name=:name,normalised_name=:normal,solo_competitor_id=COALESCE(solo_competitor_id,NULLIF(:person,0)),country=COALESCE(NULLIF(:country,''),country),photo_url=COALESCE(NULLIF(:photo,''),photo_url),original_photo_url=IF(:new_photo<>'',:new_photo2,original_photo_url),status='active' WHERE id=:id")
            ->execute(['name'=>$p['display_name'],'normal'=>$normal,'person'=>$p['person_id'],'country'=>$p['country'],'photo'=>$photo,'new_photo'=>$photo,'new_photo2'=>$photo,'id'=>$id]);
        if(!empty($p['countries']))$pdo->prepare('UPDATE bdc_wdc_identities SET country=:country,countries_json=:countries WHERE id=:id')->execute(['country'=>$p['countries'][0],'countries'=>CountrySetService::json($p['countries']),'id'=>$id]);
