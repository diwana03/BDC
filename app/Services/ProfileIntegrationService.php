...
final class ProfileIntegrationService
{
    ...
    private static function matchCompetitor(PDO $pdo,array $p):array
    {
        ...
        if($p['sdc_id']!==''){$q=$pdo->prepare("SELECT c.id FROM bdc_result_identities ri JOIN bdc_competitors c ON c.id=ri.competitor_id WHERE ri.council='sdc' AND ri.identity_code=:id AND c.status<>'archived'");$q->execute(['id'=>$p['sdc_id']]);$id=(int)($q->fetchColumn()?:0);return $id?['matched',$id,[]]:['invalid',0,[]];}
