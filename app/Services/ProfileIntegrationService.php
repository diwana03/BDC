...
final class ProfileIntegrationService
{
    ...
    private static function matchCompetitor(PDO $pdo,array $p):array
    {
        ...
        $q->execute(['name'=>$name,'email'=>$p['email']?:'__none__','instagram'=>$p['instagram']?:'__none__','phone'=>self::digits($p['phone'])?:'__none__']);$rows=array_values(array_filter($q->fetchAll(),static fn(array $r):bool=>in_array((string)$r['dance_role'],[$p['role'],'both'],true)));
    ...
    private static function applyCompetitor(PDO $pdo,array $u,array $p):array
    {
        ...
        $replace=(array)($p['replace_fields']??[]);$value=static fn(string $field,string $incoming)use($current,$replace):?string=>$incoming!==''&&((string)($current[$field]??'')===''||in_array($field==='exact_name'?'full_name':$field,$replace,true))?$incoming:($current[$field]??null);
        ...
        foreach($p['styles'] as $dance){$profile->execute(['id'=>$id,'dance'=>$dance,'role'=>$p['role']]);$category->execute(['id'=>$id,'dance'=>$dance,'category'=>$dance.'_'.($p['form_kind']==='open'?'open':'rising'),'source'=>'Profile integration API / '.$u['source_key']]);if($dance==='bachata')$pdo->prepare('UPDATE bdc_competitors SET dance_role=:role WHERE id=:id')->execute(['role'=>$p['role'],'id'=>$id]);}
