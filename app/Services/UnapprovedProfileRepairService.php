...
final class UnapprovedProfileRepairService
{
    ...
    public static function preview(PDO $pdo):array
    {
        ...
        $sql="SELECT p.competitor_id,c.bdc_id,c.exact_name,p.dance_style,p.dance_role,p.current_division,p.updated_at,
          (SELECT COUNT(*) FROM bdc_scoring_entries se JOIN bdc_scoring_rounds sr ON sr.id=se.round_id WHERE se.competitor_id=p.competitor_id AND sr.dance_style=p.dance_style) live_entries,
          (SELECT COUNT(*) FROM bdc_test_scoring_entries te JOIN bdc_test_scoring_rounds tr ON tr.id=te.round_id JOIN bdc_test_competitors tc ON tc.id=te.competitor_id WHERE tr.dance_style=p.dance_style AND (tc.id=p.competitor_id OR tc.bdc_id=c.bdc_id)) test_entries
         ...
         WHERE p.dance_style IN('bachata','salsa')
           ...
           AND NOT EXISTS(SELECT 1 FROM bdc_participant_results pr WHERE pr.competitor_id=p.competitor_id AND pr.dance_style=p.dance_style)
           AND NOT EXISTS(SELECT 1 FROM bdc_point_transactions pt WHERE pt.competitor_id=p.competitor_id AND pt.dance_style=p.dance_style)
           ...
           AND (EXISTS(SELECT 1 FROM bdc_scoring_entries se JOIN bdc_scoring_rounds sr ON sr.id=se.round_id WHERE se.competitor_id=p.competitor_id AND sr.dance_style=p.dance_style)
             OR EXISTS(SELECT 1 FROM bdc_test_scoring_entries te JOIN bdc_test_scoring_rounds tr ON tr.id=te.round_id JOIN bdc_test_competitors tc ON tc.id=te.competitor_id WHERE tr.dance_style=p.dance_style AND (tc.id=p.competitor_id OR tc.bdc_id=c.bdc_id)))
    ...
    public static function repair(PDO $pdo,int $userId):array
    {
        ...
        $approved=$pdo->prepare("SELECT (SELECT COUNT(*) FROM bdc_participant_results WHERE competitor_id=:c1 AND dance_style=:dance1)+(SELECT COUNT(*) FROM bdc_point_transactions WHERE competitor_id=:c2 AND dance_style=:dance2)");
