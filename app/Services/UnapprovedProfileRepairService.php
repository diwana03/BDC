    ...
    public static function preview(PDO $pdo):array
    {
        ...
        $sql="SELECT p.competitor_id,c.bdc_id,c.exact_name,p.dance_role,p.current_division,p.updated_at,
         ...
         WHERE p.dance_style='salsa'
           AND p.current_division IN('novice','intermediate','advanced','semi_pro','pro','professional','all_star','unknown')
    ...
    public static function repair(PDO $pdo,int $userId):array
    {
        ...
        try{
            foreach($preview['rows'] as $row){
                ...
                $evidence=json_encode(['bdc_id'=>$row['bdc_id'],'live_entries'=>(int)$row['live_entries'],'test_entries'=>(int)$row['test_entries'],'profile_updated_at'=>$row['updated_at']],JSON_UNESCAPED_SLASHES);
