...
for(const marker of ['LEFT JOIN bdc_competitors','LEFT JOIN bdc_judges','c.country,c.countries_json,c.photo_url','d.country,d.countries_json,d.country_code,d.photo_url','CountrySetService::fromRow','CountryFlagService::emoji'])assert(feed.includes(marker),'projection feed missing identity field '+marker);
...
assert(display.includes("<h2>'+esc(data.state.event_name||'Dance Cup')+'</h2>"),'holding screen must feature event name');
