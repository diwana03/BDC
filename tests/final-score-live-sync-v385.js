...
for(const path of ['admin/scoring-tests/index.php','admin/scoring/core.php']){
 ...
 const match=[...source.matchAll(/final-score-sync\.js\?v=(?:38[5-9]|[4-9]\d{2,})/g)].at(-1);
 assert(match);
 assert(match.index<source.lastIndexOf('</body>'));
}
...
assert(sync.includes('setInterval(refresh,5000)'));
