...
assert(outer.includes("projector-roster-v615.css?v=641"), 'Mobile stylesheet cache key is stale');
assert(feed.includes('$competitorRolePage=$isFlightRoster?1:'), 'Flight number must not be reused as a roster page offset');
assert(feed.includes('if(!$isFlightRoster){'), 'Flight contestants must bypass ordinary roster pagination');
