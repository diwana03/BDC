...
$checks = [
    ...
    'no public sound toggle' => !str_contains($sound, 'Enable Sound'),
    'sound cache refreshed' => str_contains($display, 'effect-sound.js?v=303'),
    'projection volume control' => str_contains($control, 'projectorVolume') && str_contains($sound, 'requestedVolume'),
    'sound gate only in sound mode' => str_contains($sound, 'if (soundRequested)') && str_contains($sound, 'projectorAudioGate'),
    'browser activation instruction' => str_contains($control, 'one-time Start Projector gate'),
    'audio cannot interrupt projector renderer' => !str_contains($display, 'bdc-projector-effect'),
    'late audio resumes pending effect' => str_contains($sound, 'pendingEffect') && str_contains($sound, "audio?.state === 'running'"),
    'podium reveal has no automatic drumroll' => !str_contains($control, 'drumroll_'),
    'podium panel confirms manual effects' => str_contains($control, 'No effects play automatically.'),
