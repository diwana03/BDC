...
assert(publish.includes("fetchReportPreview(finalSource+'&layout=fit')"), 'publication must capture the all-judge landscape Final');
assert(publish.includes('fetchReportPreview(finalSource)'), 'publication must capture the readable Final');
...
assert(publish.includes("action\" value=\"refresh_result_archives\""), 'published special reports must be refreshable without republishing points');
assert(publish.includes("'/.archive-backups'"), 'refresh must back up the existing published Final first');
assert(publish.includes("'previous'=>hash_file") && publish.includes("'new'=>hash_file"), 'refresh must audit old and new archive checksums');
