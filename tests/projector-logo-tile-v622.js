...
assert(feed.includes('projector-safe-v616.css?v=<?='), 'projector logo CSS must use a dynamic cache key');
...
assert(safe.includes('margin:0 !important'), 'logo must not introduce asymmetric heading margins');
assert(safe.includes('align-self:center'), 'logo is not vertically centered with the heading');
