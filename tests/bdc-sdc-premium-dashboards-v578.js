...
assert(page.includes("$dashboard === 'salsa' ? 'sdc' : 'bdc'")&&page.includes("SELECT id competitor_id,bdc_id identity_code FROM bdc_competitors WHERE bdc_id LIKE 'BDC-%'")&&page.includes('FROM bdc_sdc_competitors WHERE status=\'active\''),'council identity isolation changed');
