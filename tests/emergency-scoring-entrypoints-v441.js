...
for(const marker of ['information_schema.TABLES','$wdcReady','Dance Participation','Some WDC participant data could not be loaded:'])assert(participants.includes(marker),'Participant fallback missing '+marker);
...
assert(participants.includes('WDC identities and registrations are not available yet.')&&participants.includes('Some WDC participant data could not be loaded:'),'Participants migration fallback must render a normal recovery page');
