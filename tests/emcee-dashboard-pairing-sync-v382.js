...
for(const path of ['admin/scoring-tests/index.php','admin/scoring/core.php']){const s=fs.readFileSync(path,'utf8');assert(s.includes('data-final-leader-id'));assert(s.includes('data-pairing-state-url'));assert(/final-pairing-sync\.js\?v=(?:38[2-9]|[4-9]\d{2,})/.test(s));assert(s.includes('Emcee Random Match'));}
