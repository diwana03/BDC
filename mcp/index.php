...
$header=trim((string)($_SERVER['HTTP_AUTHORIZATION']??''));$bearer=preg_match('/^Bearer\s+(.+)$/i',$header,$m)?trim($m[1]):'';$method=(string)($req['method']??'');$mutatingTools=['stage_competitor_additions','stage_competitor_photo_update'];$required=$method==='tools/call'&&in_array((string)($req['params']['name']??''),$mutatingTools,true)?McpOAuthService::STAGE_SCOPE:McpOAuthService::READ_SCOPE;$user=McpOAuthService::authenticate(Database::connection(),$bearer,$required);
