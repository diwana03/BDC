...
$versionManifest=json_decode((string)@file_get_contents(dirname(__DIR__).'/VERSION.json'),true);$serverVersion=(string)($versionManifest['version']??'unknown');
if(($_SERVER['REQUEST_METHOD']??'')!=='POST')mcpOut(['error'=>'POST required.'],405);
$raw=(string)file_get_contents('php://input');if(strlen($raw)>22*1024*1024)mcpOut(['error'=>'Payload too large.'],413);$req=json_decode($raw,true);if(!is_array($req))mcpOut(['error'=>'Invalid JSON.'],400);
$header=trim((string)($_SERVER['HTTP_AUTHORIZATION']??$_SERVER['REDIRECT_HTTP_AUTHORIZATION']??''));if($header===''&&function_exists('getallheaders')){$headers=getallheaders();if(is_array($headers))foreach($headers as $k=>$v)if(strcasecmp((string)$k,'Authorization')===0){$header=trim((string)$v);break;}}$bearer=preg_match('/^Bearer\s+(.+)$/i',$header,$m)?trim($m[1]):'';$method=(string)($req['method']??'');$mutatingTools=['stage_competitor_additions','stage_competitor_photo_update'];$required=$method==='tools/call'&&in_array((string)($req['params']['name']??''),$mutatingTools,true)?McpOAuthService::STAGE_SCOPE:McpOAuthService::READ_SCOPE;$user=McpOAuthService::authenticate(Database::connection(),$bearer,$required);
...
$id=$req['id']??null;
if($method==='initialize')mcpOut(['jsonrpc'=>'2.0','id'=>$id,'result'=>['protocolVersion'=>'2025-06-18','capabilities'=>['tools'=>['listChanged'=>false]],'serverInfo'=>['name'=>'BDC Portal','version'=>$serverVersion]]]);
