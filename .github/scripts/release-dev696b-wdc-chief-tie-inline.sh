...
actual='<main class="container py-4" style="max-width:1100px"><?php if($notice):?>'
replacement='<main class="container py-4" style="max-width:1100px"><?php if($chiefTieNotice!==\'\'):?><div class="alert alert-success"><?=e($chiefTieNotice)?></div><?php endif;?><?=$chiefTieHtml?><?php if($notice):?>'
if '<?=$chiefTieHtml?><?php if($notice):?>' not in s:
    if actual not in s: raise SystemExit('actual judge template main anchor missing')
    s=s.replace(actual,replacement,1)
p.write_text(s)
...
assert '<?=$chiefTieHtml?><?php if($notice):?>' in s
print('dev696b WDC Chief Judge inline tie assertions passed')
PY
