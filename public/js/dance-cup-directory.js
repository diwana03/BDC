(function(){
 ...
 function attach(input){
  const type=input.dataset.directoryType,hidden=document.getElementById(input.dataset.directoryTarget),wdcHidden=input.dataset.directoryWdcTarget?document.getElementById(input.dataset.directoryWdcTarget):null,wrap=input.closest('.dc-directory-field');
  if(!type||!hidden||!wrap)return;
  const menu=document.createElement('div');menu.className='dc-directory-menu';menu.hidden=true;wrap.appendChild(menu);const card=wrap.closest('.card,section');let timer,controller;
  const open=()=>{menu.hidden=false;wrap.classList.add('dc-directory-open');card?.classList.add('dc-directory-card-open')};
  const close=()=>{menu.hidden=true;menu.innerHTML='';wrap.classList.remove('dc-directory-open');card?.classList.remove('dc-directory-card-open')};
  const choose=item=>{input.value=item.name;hidden.value=item.id;if(wdcHidden)wdcHidden.value=item.wdc_identity_id||'';input.classList.add('dc-directory-selected');close();input.focus()};
  const search=()=>{hidden.value='';if(wdcHidden)wdcHidden.value='';input.classList.remove('dc-directory-selected');clearTimeout(timer);const q=input.value.trim();if(q.length<1){close();return;}timer=setTimeout(async()=>{controller?.abort();controller=new AbortController();try{const endpoint=new URL('directory-search.php',location.href);endpoint.searchParams.set('type',type);endpoint.searchParams.set('q',q);if(type==='competitor'){const params=new URLSearchParams(location.search);if(params.get('id'))endpoint.searchParams.set('competition_id',params.get('id'));if(params.get('data_mode')==='test')endpoint.searchParams.set('data_mode','test');}const response=await fetch(endpoint,{cache:'no-store',headers:{Accept:'application/json'},signal:controller.signal});let data;try{data=await response.json()}catch{throw new Error('invalid-response')}if(!response.ok||!data.ok||!Array.isArray(data.items))throw new Error('search-failed');menu.innerHTML=data.items.length?data.items.map((item,index)=>`<button type="button" data-index="${index}"><strong>${escapeHtml(item.name)}</strong><small>${escapeHtml(item.meta||'BDC Directory')}</small></button>`).join(''):'<div class="dc-directory-empty">No directory match. You may enter a new name.</div>';open();menu.querySelectorAll('button').forEach(button=>button.onclick=()=>choose(data.items[Number(button.dataset.index)]));}catch(error){if(error.name!=='AbortError'){menu.innerHTML='<div class="dc-directory-empty">BDC Directory search could not load. Please retry.</div>';open();}}},180)};
